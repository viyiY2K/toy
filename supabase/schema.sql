...
-- ============================================================
-- Row Level Security：每张表只允许访问自己的行；七张实体表禁止物理删除，
...
-- 七张可同步实体表：select / insert / update 允许，delete 一律拒绝（软删除代替物理删除）。
do $$
declare
  t text;
begin
  foreach t in array array['tasks','day_plans','sessions','energy_records','unresolved_intervals','settings','merge_groups']
  loop
