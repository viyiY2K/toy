...
> **正式基准文档。本文件取代 v1 / v2 / v2.1 / v3 / v4.0（`data-layer-spec-v4.md`）/ v4.1（`data-layer-spec-v4.1.md`）/ v4.2（`data-layer-spec-v4.2.md`）。后续所有开发仅参考本文件（v4.3.2）。**
> 版本：4.3.2 | 制定日期：2026-05-29 | 最近修订：2026-08-10（**4.3.2**：整合后续规范结论——`Task.mergeGroupId` 严格表示当前所属，MergeGroup 进入 `completed` / `dissolved` 终态时清空全部成员指针；`mergeGroup.completed` 顶层新增触发确认的 `sessionId`，payload 扩展为 `completedAt` / `validFocusCountAtCompletion` / `finalTaskIds` / `incompleteTaskIds`，允许仍有未完成成员时确认整组完成，并按“开轮至少 2 个未完成成员、已开跑 Session 与终结快照允许缩到 1”统一约束；§14 V43-1～V43-6 全部结案。**4.3.1**：合并番茄钟产品语义收口——活动 focus Session 锁定当前执行对象；开启任何一轮合并 focus 前未完成成员必须 ≥2，但 Session 存储约束保持 `taskIds ≥ 1`；MergeGroup 保留成功终态 `completed`、`renamed` 与 §7.19 共 8 个 `mergeGroup.*` 事件；成员耗时改用 `taskSegments`，番茄归组、时间归成员。**4.3.0**：新增 Goal、废除子任务机制并改写合并番茄钟统计口径。`data-layer-spec-v4.2.md` 及更早版本保留作历史归档）
...
| 历史版本 | 主要贡献 | v4 中的状态 |
|---|---|---|
| v1 | 初版数据模型草稿，确立了扁平 Task + parentId 思路 | 已归档；其中 `parentId` 子任务思路已于 v4.3 整体废除，见 §3.1 |
...
| v4.0.0 | 自包含正式基准，新增跨端契约、可运营性章节，修订若干歧义 | 已归档，内容已内嵌进 v4.1 / v4.2 |
| v4.1.0 | 新增"合并番茄钟"能力：MergeGroup 实体（§3.8）、Session `taskId`→`taskIds` 结构调整（§3.3）、Task `mergeGroupId` 字段（§3.1）、6 个 mergeGroup.* 事件（§7.19）、新增 promptType='mergeGroupLimitReached'（§7.15）、对应统计口径补充（§8.3、§8.5） | 已归档，内容已内嵌进 v4.2 |
| v4.2.0 | 新增"任务分类"能力：Task `category` / `categorySource` 字段（§3.1）、DayPlan `mainCategory` / `mainCategorySource` 今日主线分类字段（§3.2）、Settings `taskCategoryWindows` 自动打标时间窗口配置（§3.7）、`task.categoryChanged` / `dayPlan.mainCategoryChanged` / `settings.taskCategoryWindowsChanged` 三个事件（§7.1、§7.3、§7.12）、按分类统计投入时长口径（§8.5.6） | 已归档，内容已内嵌进 v4.3 |
| v4.3.0 | 新增"中长期主线目标"能力：Goal 实体（§3.9）、Task `goalId`（§3.1）、9 个 `goal.*` 事件（§7.20）、`task.goalLinked` / `task.goalUnlinked`（§7.4）、Goal 维度统计（§8.13）；废除子任务机制（`parentId`、`subtask.*`、`task.reparented`）；改写合并番茄钟统计口径（Session `taskSegments` 分段、MergeGroup `title` 与 `mergeGroup.renamed`，§3.3、§3.8、§7.19、§8.3、§8.5） | 已并入 v4.3.1 |
| v4.3.1 | 合并番茄钟产品语义收口：活动 focus Session 锁定当前执行对象；开轮前未完成成员 ≥2、Session 存储约束保持 `taskIds ≥1`；保留成功终态 `completed`、`renamed` 与 8 个 `mergeGroup.*` 事件；补齐 `taskSegments` 与 MergeGroup 统计口径 | 已并入 v4.3.2 |
| **v4.3.2（本文）** | 整合 V43-3～V43-6 最终结论：终态清空成员当前归属指针；`mergeGroup.completed` 增加顶层 `sessionId` 与成员终结快照，允许仍有未完成成员时确认完成；completed / dissolved 终态与重命名例外统一；保留 v4.3.1 的开轮 ≥2 / 已开跑记录 ≥1 分层规则 | **唯一有效版本** |

**声明**：自本文件制定之日起，v1 / v2 / v2.1 / v3 / v4.0 / v4.1 / v4.2 均降级为历史归档，仅供溯源查阅，**不再作为任何开发决策的依据**。如历史版本内容与本文有出入，以本文为准。v4.0.0、v4.1.0 与 v4.2.0 的内容已完整内嵌入本文（v4.3.0），未被 v4.3 修订的部分保持不变。
...
Task 是产品的核心实体，表示一个**独立任务**——单天内能做完、可以被排进今日待办、可以直接开始番茄计时的具体事项。Task **不存在层级关系**：任务之间一律平等，没有母任务与子任务之分（v4.3 起，见下方"关于 `parentId` 的废弃说明"）。跨周 / 跨月的中长期目标不是 Task，而是独立实体 Goal（§3.9），Task 通过 `goalId` 向上关联到某个 Goal。番茄数、专注时长等统计数据**不存在 Task 上**，一律从 Session 和 Event 派生；具体统计口径见 §8。

> **关于 `parentId` 的废弃说明（v4.3）**：v4.0–v4.2 的 Task 曾有 `parentId` 字段，用于表达"母任务 / 子任务"两层结构。v4.3 起该字段与配套的 `subtask.*` 事件域、`task.reparented` 事件**整体废弃并从规范中移除**：该机制与 Goal（§3.9）的定位重叠，两套并存会让"一件大事下面挂着的小事"出现两种互不相通的表达方式。废弃时产品尚未正式投入使用，无历史数据需要保留，因此不设迁移路径、不保留兼容字段——实现端应直接删除该字段与相关事件类型，而不是标记为 deprecated 后继续携带。
...
| `goalId` | `string \| null` | 是 | `null` | 所关联的中长期主线目标（Goal，见 §3.9）id（v4.3 新增）；`null` = 未关联任何 Goal，是一件不隶属于任何中长期目标的独立任务；有值表示本任务是推进该 Goal 的一次具体行动。一个 Task 最多只能关联一个 Goal（互斥，不支持多关联）；关联与解除关联可在任务生命周期的任意时刻发生，**包括任务已完成或已归档之后**（见 §7.4 `task.goalLinked` / `task.goalUnlinked`）；取值约束：null 或合法的 Goal UUID v7，允许指向已软删除的 Goal（见 §3.9 关键规则 6） |
...
| `mergeGroupId` | `string \| null` | 是 | `null` | **当前**所属合并组（MergeGroup，见 §3.8）id；`null` 表示当前未参与任何合并组；有值表示该任务此刻正被并入某个进行中的合并番茄钟，与其他成员地位平等。本字段严格表示“当前所属”，**不承载历史来源**：MergeGroup 进入 `completed` 或 `dissolved` 终态时，全部成员的本字段必须清空；历史归属由 `MergeGroup.taskIds`、`Session.mergeGroupId`、`Session.taskSegments` 与 Event 关联共同承载，查询不得依赖本字段。取值约束：null 或合法的 MergeGroup UUID v7；非 null 时该组必须包含本 Task，且状态必须为 `active` 或 `limitReached` |
...
12. **`goalId`（中长期目标归属）与 `mergeGroupId`（合并组归属）是两个互不影响的维度，可以同时非 null（v4.3 修订）**。`goalId` 表达"这件事是在推进哪个中长期目标"，是**语义归属**，长期有效，用于回溯某个目标累计投入了多少任务与多少时间；`mergeGroupId` 表达"这个任务这次和其他任务被合并进同一个番茄钟一起做"，纯粹是**执行层面的临时归并**，只说明这几件事各自都不足以占满一个番茄，不代表它们属于同一件事。因此：**同一个合并组内的成员，可以各自关联到不同的 Goal，也可以有的关联、有的不关联**；两个字段的写入与清空互不联动，任何实现都不得从"同属一个合并组"推断"同属一个 Goal"，反之亦然。
13. **`category` / `categorySource` 是独立于 `goalId` / `mergeGroupId` 的第三个维度（v4.2 新增，v4.3 修订表述）**，用于粗粒度统计"工作 / 学习 / 副业 / 生活"投入时间比例；不是项目管理意义上的"项目"或"清单"，不提供改名、层级、颜色等扩展属性，就是一个受控枚举字段。自动判定的触发时机、判定算法与手动覆盖优先级见 §7.1 `task.categoryChanged`；今日主线分类的默认预填规则见 §3.2 DayPlan 关键规则第 11 条。
14. **`Task.category` 与其所属 Goal 的 `category` 相互独立，不做任何强制一致性校验（v4.3 新增）**。Goal 也有自己的 `category` 字段（见 §3.9），但二者是两条独立的取值：一个 `category='work'` 的 Goal 下面，完全可能挂着一个被自动判定或用户手动打成 `'life'` 的 Task。数据层**不校验、不联动、不追溯覆盖**——关联 Goal 不改写 Task 已有的 `category`，修改 Goal 的 `category` 也不追溯改写其名下任何 Task。统计时两者各按各的口径走：按分类统计走 §8.5.6（以 `Task.category` 为准），按 Goal 统计走 §8.13（以 `Task.goalId` 为准）。
...
5. **（v4.3 修订）** `goalId` 非 null 时，其值必须能在 Goal 表中找到对应记录（含已软删除的 Goal，见 §3.9 关键规则 6）；找不到对应 Goal 的写入视为悬空引用，必须被拒绝。Task 之间不存在任何父子 / 层级引用字段——v4.0–v4.2 的 `parentId` 及其 2 层上限约束已随该字段一并废除。
...
| `taskIds` | `string[]` | 否 | `[]` | 关联任务 id 列表；focus 时长度必须 ≥ 1（不允许空数组，产品不支持无任务自由专注）——单任务专注时数组只有 1 个元素；合并番茄钟开轮时至少 2 个未完成成员，但已开跑记录可合法缩到 1，元素间完全平等、不区分主次（见 §3.8 MergeGroup）；extraFocus 不支持合并，长度固定为 1；shortBreak / longBreak / extraRest 固定为空数组 `[]`；取值约束：数组元素均为合法 Task UUID v7，数组内不允许重复 |
| `mergeGroupId` | `string \| null` | 是 | `null` | 若本次专注由某个合并组（MergeGroup，见 §3.8）触发，则指向该合并组 id；普通单任务专注、extraFocus 与全部休息类型固定为 null。取值约束：null 或合法的 MergeGroup UUID v7；非 null 时 `taskIds` 长度必须 ≥ 1。**开轮命令**另行要求开始时至少 2 个未完成成员；这里是**已开跑 Session 的存储约束**，必须允许进行中移出未来成员后缩到 1 并正常收尾（见 §3.8 关键规则 15/16） |
| `taskSegments` | `array` | 否 | `[]` | **成员分段（v4.3 新增）**：合并番茄钟场景下，本次专注的实际时长按组内成员的执行先后顺序切分而成的分段明细，结构见下方 taskSegments 数组元素结构；这是"某个成员任务在这次合并番茄里实际花了多少时间"的**唯一事实源**（切分规则见关键规则 12、统计口径见 §8.5）；`mergeGroupId` 为 null 的单任务专注、以及 extraFocus / 全部休息类型固定为空数组 `[]`（单任务专注的任务耗时即 `actualDuration` 本身，无需分段）；Session 终结时一次性写入，写入后固定不再变化；取值约束：数组（可为空），元素须符合下方 taskSegments 结构定义；非空时各元素 `actualDuration` 之和必须**精确等于**本 Session 的 `actualDuration`（见字段一致性约束 16） |
...
| `pomodoroIndex` | `number \| null` | 是 | `null` | 普通独立 focus（`mergeGroupId=null`）时，是该 Task 的 focus 发生序号；**合并 focus（`mergeGroupId` 非 null）时，是该 MergeGroup 的发生序号**，即使已开跑后 `taskIds` 缩到 1 也不改变归属、不退化为独立 Task 序号。shortBreak / longBreak / extraFocus / extraRest 固定 null；discarded focus 也占用序号，不回收；取值约束：正整数（≥1）或 null |
...
**taskSegments 数组元素结构（v4.3 新增）**

```
{
  taskId:         string   本分段对应的成员 Task id；取值约束：合法 Task UUID v7，
                           且必须出现在本 Session 的 taskIds 数组中；同一 Session 内
                           不允许出现两条 taskId 相同的分段
  startedAt:      string   本成员开始被推进的时刻；取值约束：ISO 8601 带时区格式
  endedAt:        string   本成员结束被推进的时刻（被勾完成，或本次 Session 终结）；
                           取值约束：ISO 8601 带时区格式，且不早于本分段 startedAt
  actualDuration: number   本成员在这次专注里的实际耗时，单位秒；取值约束：整数，≥ 0
                           （从未轮到的成员为 0，见关键规则 13）
}
```

> 数组顺序即成员的实际执行先后顺序，与 Session 终结时 `MergeGroup.taskIds` 的展示顺序一致。与 §3.3 关键规则 10 同理，`actualDuration` 是本分段耗时的**唯一事实源**，统计不得用 `endedAt − startedAt` 重算。
...
| `taskSegments` | 当前版本新写入中，`mergeGroupId` 非 null 且已终结时必填（长度 = `taskIds` 长度，≥ 1）；历史迁移例外保持 `[]`（见关键规则 13）；否则不适用（固定 `[]`） | 不适用（固定 `[]`） | 不适用（固定 `[]`） | 不适用（固定 `[]`） | 不适用（固定 `[]`） |
...
12. **合并番茄钟的番茄归属：番茄记在合并组身上，不记在成员任务身上（v4.3 改写，推翻 v4.1/v4.2 口径）**。一次由 MergeGroup 触发的 focus Session 正常完成时：
    - **该合并组**记 1 个完整的有效标准 focus，本次的 `actualDuration` 全额计入该合并组的专注时长；
    - **组内各成员 Task 各自的有效番茄数不增加**（一个都不加，不是每人加 1，也不是平分成小数）。成员任务在这次专注里的收获，只体现为 `taskSegments` 里属于它的那一段实际耗时。

    > **v4.1 / v4.2 的旧口径已作废**：旧口径规定"每个成员各自记一个完整有效番茄、各自完整计入整段 `actualDuration`，不做平分"，并接受"任务维度加总 > 全局总数"。v4.3 起不再采用——旧口径下同一段专注时间会在多个维度被重复计入，一旦成员任务分属不同 Goal（§3.9），各 Goal 的投入时长加总就会显著超过用户当天真实的专注时间，使 Goal 维度的统计失去意义。新口径下，合并组是番茄的归属单位，成员是时间的归属单位，两者不重叠，任何维度加总都不会超过真实投入。展示上，统计页默认呈现的是合并组这个整体，成员各自的耗时与时间段需展开该合并组的详情才可见（见 §8.5）。

13. **`taskSegments` 的切分规则（v4.3 新增）**。分段的事实来源是"用户在专注进行中逐个勾选组内成员完成"这一真实操作；Session 终结时按下列规则一次性算出并写入，此后固定不变：
    - **第一个成员**的 `startedAt` = 本 Session 的 `startedAt`；
    - **其后每个成员**的 `startedAt` = 上一个成员的 `endedAt`（即前一件事被勾完成的时刻，就是后一件事开始的时刻）；
    - **已被勾完成的成员**，其 `endedAt` = 被勾完成的时刻；
    - Session 终结时**仍未完成的成员**中，只有**当前正在做的那一个**（即最后一个被勾完成的成员之后紧接着的那一个）拿走剩余时间，其 `endedAt` = 本 Session 的 `endedAt`；**再往后尚未轮到的成员**，`startedAt` 与 `endedAt` 均等于本 Session 的 `endedAt`，`actualDuration` 记 `0`；
    - **一个成员都没有被勾完成**的极端情况：整段时长全部归第一个成员，其余成员 `actualDuration` 均记 `0`；
    - 各分段 `actualDuration` 之和必须**精确等于**本 Session 的 `actualDuration`，不允许出现对不上的零头（见字段一致性约束 16）。若因取整产生 1 秒以内的余数，一律并入**最后一个 `actualDuration` 大于 0 的分段**，不做平均分摊。

    **「当前成员」的判定（v4.3 修订）**：本轮的当前成员 = 按 `MergeGroup.taskIds` 顺序排列的本轮参与成员中，**第一个尚未完成、且未在更早轮次取得处理结果的成员**。本轮开始时它是队首；每当它被勾完成，它的分段就地结束，紧随其后的未来成员立刻接任为当前成员，其分段从同一时刻起算。**实现不得直接用字面上的 `taskIds[0]` 判定当前成员**——已完成的历史成员仍然留在 `MergeGroup.taskIds` 里（见 §3.8 关键规则 4），队首很可能是一个早就完成的成员。

    **中途加入的成员从哪一刻起算（v4.3 修订）**：加入时刻**不是**计时起点。新成员只有真正轮到它时才开始分段——起点 = 它在执行顺序中前一位成员的结束时刻，终点 = 它被勾完成的时刻或本 Session 的终结时刻。本轮始终没轮到它时，它的实际投入为 `0`，**不得**把它加入前的时间、或它在队列里等待的时间分配给它。

    **`actualDuration` 与时间戳的关系**：分段边界取真实时间戳，但每段的 `actualDuration` 必须按本 Session 的 `actualDuration` 等比切分后取整，使各段之和精确等于 Session 总时长（Session 的 `actualDuration` 与 `endedAt − startedAt` 因倒计时漂移可能不等，见关键规则 10）。写入后 `actualDuration` 即为该分段耗时的唯一事实源，统计**不得**再用 `endedAt − startedAt` 重算。

    Session 终结后 `taskSegments` 固定，**不随此后 MergeGroup 成员的任何变化而改写**（Session 是历史事实，见关键规则 11）。

    本规则同样适用于 `status='discarded'` 的合并 focus：作废的合并专注仍写入 `taskSegments`（用户确实投入了这段时间），当前正在执行的成员的分段结束于作废时刻，尚未轮到的成员一律记 `0`、不得分到等待时间；其分段耗时按 §8.3.5 计入各成员任务的作废专注时长，不计入全局有效番茄数、不计入 MergeGroup 有效番茄数、也不计入任何成员任务的有效番茄数。

    **历史迁移例外**：从旧 schema 迁入、且当时没有逐个成员完成时刻可供还原的历史合并 Session，必须保持 `taskSegments: []`。迁移不得伪造平均分摊或其他推测分段；统计侧将这类 Session 对各成员的时长贡献按 `0` 计算。此例外只适用于无法还原分段事实的历史迁移记录，不放宽当前版本新终结 Session 的完整分段写入约束。

14. **活动 focus Session 对其执行对象的锁定（v4.3 新增）**。一条 `status='active'` 的 focus Session 会锁定它当前正在执行的对象，锁定期间数据层**必须拒绝**改变该对象的普通写入——这条约束由数据层的每一条相关 command / write path 强制保证，**不得只靠 UI 隐藏或禁用按钮**。

    - **独立任务专注**（`mergeGroupId` 为 null）：被引用的那个 Task 处于锁定态。锁定期间必须拒绝：删除 / 软删除该 Task；从计划页直接修改预估番茄数；绕过正式计时流程调整 `estimatedPomodoros` / `estimateRounds`；以及其他会改变本轮执行对象或任务核心状态的普通修改。
    - **合并番茄专注**（`mergeGroupId` 非 null）：锁定的是本轮的**当前成员**（判定见关键规则 13）。锁定期间必须拒绝：把当前成员移出合并组；把当前成员排序到其他位置；把未来成员移动到当前成员之前；通过整体解散合并组间接移出当前成员；以及对当前成员的删除、预估调整等普通任务修改。**当前成员之后、尚未开始执行的未来成员不受锁定**——允许新增、移出，以及在未来队列内部相互排序；中途新增的成员一律排在当前成员之后，不得插到当前成员之前。当前成员本身**允许**通过合并番茄专用的勾选完成流程结束其分段并推进到下一位（见 §3.8 关键规则 11）。

    **解除锁定只有两条路径**：（1）当前 focus 被作废为 `discarded`，此后任务恢复普通可编辑状态；（2）当前 focus 正常到点成为 `completed`，此后只能通过正式的收尾 / 重新预估流程调整预估，**不得**把计划页随意改数字当作重新预估。

    产品**不存在暂停 / 恢复**功能：标准 focus 的状态只有 `'active'` / `'completed'` / `'discarded'` 三种。番茄开始后无论已经进行多久，中途终止都只能作废为 `'discarded'`；不存在「5 分钟以内可暂停」「超过 5 分钟不能暂停」之类的状态边界，5 分钟不参与任何状态机判定。若独立任务在计时中提前完成，应先按本规则作废当前番茄、再走相应的完成流程，**不得**一边保持 Session `active`、一边直接修改或删除当前任务。
...
15. `mergeGroupId` 非 null 时，`type` 必须为 `'focus'`，且 `taskIds` 长度必须 ≥ 1。反向亦然：`type='focus'` 且 `taskIds` 长度 ≥ 2 时，`mergeGroupId` 必须非 null（多任务专注只可能来自合并组）。
    ...
    > **不得把这里收紧回 ≥ 2**。§3.8 关键规则 16 要求开轮命令在开始时校验未完成成员至少 2 个；本条只校验已经存在的 Session。合法开跑后，用户可在 active Session 期间移出未来成员，使 `taskIds` 与终结快照缩到 1；该轮仍带原 `mergeGroupId`、继续使用合并组轮次，且必须能够正常写入 `taskSegments` 并收尾，不退化为普通独立 Task focus。
16. **（v4.3 新增）** `taskSegments` 的适用性与内部一致性：
    - `mergeGroupId` 为 null 时（含全部非 focus type），`taskSegments` 必须为空数组 `[]`；
    - `mergeGroupId` 非 null 且 `status='active'` 时，`taskSegments` 必须为空数组 `[]`（分段在 Session 终结时才一次性写入）；
    - 当前版本新终结写入中，`mergeGroupId` 非 null 且 `status ∈ {'completed', 'discarded'}` 时，`taskSegments` 长度必须与 `taskIds` 长度相等，且两者的 taskId 集合必须**完全一致**（每个参与本轮的成员都必须有且只有一条分段记录，即使其耗时为 0）；
    - 各分段的 `actualDuration` 之和必须**精确等于**本 Session 的 `actualDuration`；不相等的写入必须被拒绝；
    - 每条分段的 `endedAt` 不得早于其 `startedAt`；数组内不允许出现重复的 `taskId`。
    - 关键规则 13 所述历史迁移 Session 是唯一例外：无法还原分段事实时允许 `taskSegments: []`，成员贡献按 0；不得据此放宽任何当前版本业务写入。
...
MergeGroup 表示"合并番茄钟"：把清单页里几个耗时不足一个番茄的零碎任务拖到一起，用**一段**专注时间依次推进——产品语言里可以称之为一个"杂事番茄"。**这一个番茄记在合并组身上，不记在成员任务身上**；成员任务各自只记它在这段时间里实际占用的那一小段（见 §3.3 关键规则 12、13 与 `taskSegments` 字段）。

这是**平等的集合关系**：成员之间没有主次，也**不代表它们属于同一件事**——合并只说明"这几件事各自都不足以占满一个番茄"，纯粹是执行层面的临时归并。因此**同一个合并组内的成员，可以各自关联到不同的 Goal（§3.9），也可以有的关联、有的不关联**；任何实现都不得从"同属一个合并组"推断"同属一个中长期目标"（见 §3.1 关键规则 12）。

**MergeGroup 是与独立 Task 同级的一个番茄执行任务，不是可无限复用的长期容器。**它有初始预估、可追加预估、完成确认与成功终态；用户确认完成后必须新建另一组来承载下一批杂事，不得复用旧组。它仍是独立的 MergeGroup 实体，不伪造成 Task。
...
|---|---|---|---|---|
| `id` | `string`（UUID v7） | 否 | 写入时生成 | 实体唯一标识；取值约束：UUID v7 格式 |
| `title` | `string` | 否 | 创建时由系统生成的默认名（如"杂事番茄"） | **合并组名称（v4.3 新增）**：这一个"杂事番茄"在统计页与历史记录里的显示名。新口径下合并组本身是番茄与专注时长的归属单位（见 §3.3 关键规则 12），会作为一个独立条目出现在统计与历史列表中，因此必须有名字，否则多个合并组无法区分。创建时由系统写入默认名，用户可随时重命名（→ `mergeGroup.renamed`，见 §7.19）；取值约束：非空字符串（不允许空字符串或纯空白），最大长度 200 字符（与 §3.1 Task.title 一致） |
| `taskIds` | `string[]` | 否 | 创建时的初始成员 | 组内成员 Task id 列表，有序，数组顺序即展示与推进顺序（决定 §3.3 `taskSegments` 切分）；是持续的成员名单，不因成员完成自动移除；进入终态后固定为终结成员快照，供历史回溯。取值约束：元素均为合法 Task UUID v7、不得重复；`active` / `limitReached` 常态长度 ≥2，活动 Session 期间可暂时缩到 1；`completed` 终结记录允许长度 ≥1 |
...
| `status` | `string`（枚举） | 否 | `'active'` | 合并组状态；`'active'` = 进行中；`'limitReached'` = 已达到预估 / 番茄数硬上限且组内仍有未完成成员，被阻塞；`'completed'` = 用户已明确确认这一组做完了（**v4.3 新增**，成功终态）；`'dissolved'` = 已解散（中途拆散 / 取消合并，**不代表成功完成**）；取值约束：`'active'` / `'limitReached'` / `'completed'` / `'dissolved'` 之一 |
| `completedAt` | `string \| null` | 是 | `null` | **（v4.3 新增）** 用户确认这一组完成的时刻；`status='completed'` 时必须非 null，否则必须为 null；取值约束：ISO 8601 带时区格式或 null |
...
| 值 | 含义 |
|---|---|
| `'active'` | 进行中，`taskIds` 长度 ≥ 2（唯一例外见关键规则 15），可继续增删成员、追加预估、发起 focus Session |
...
| `'completed'` | 用户已明确确认这一组做完了，是合并组的**成功终态**。全部成员的 `Task.mergeGroupId` 已清空；除重命名外不允许任何 MergeGroup 业务写入（见关键规则 12） |
| `'dissolved'` | 已解散的未成功终态。全部成员的 `Task.mergeGroupId` 已清空；历史记录保留供回溯。它只表示中途拆散 / 取消合并，不代表成功完成；除重命名外不允许任何 MergeGroup 业务写入 |
...
1. `taskIds` 是有序数组，顺序即合并卡片内的展示与推进顺序；可动态增删：加入触发 `mergeGroup.taskAdded`，移出触发 `mergeGroup.taskRemoved`（见 §7.19）。**建组门槛与每一轮开轮门槛都要求至少 2 个未完成成员**；但开跑后的 Session 存储合法性仍是 `taskIds ≥1`，活动期间只允许编辑未来队列，当前成员被锁定（详见关键规则 15/16 与 §3.3 一致性约束 15）。
2. `taskIds` 长度掉到 1（含）以下时，MergeGroup 自动解散（`dissolvedReason='membersBelowMinimum'`），若还剩最后 1 个任务，其 `Task.mergeGroupId` 一并清空。**唯一例外**：该组正有一条 `status='active'` 的 focus Session 时，自动解散**推迟**到本轮结算，见关键规则 15。
...
4. 一轮标准 focus 正常结束、对应休息流程收尾后，用户在三条路径中显式选择：
   - **确认本合并组完成**：允许仍有未完成成员；原子写入 `status='completed'`、`completedAt`，清空全部成员当前归属指针并追加 `mergeGroup.completed`。未完成成员保持 `active`，之后作为独立任务继续处理。
   - **追加预估继续**：合并组保持 `active`，更新 `estimatedPomodoros` / `estimateRounds`；但真正开启下一轮前仍必须重新满足至少 2 个未完成成员，不能因为已追加预估就绕过关键规则 16。
   - **放弃本次合并**：未完成成员可逐个移出（`reason='sessionEndedIncomplete'`），必要时按成员不足转入 `dissolved`；也可走主动整体解散。解散不是成功完成。
...
8. 组内成员支持调整先后顺序（一个番茄内先做哪件杂事、后做哪件，与活动清单 / 今日待办的顶层排序是类似逻辑）：拖拽调整只重排 `taskIds` 数组，触发 `mergeGroup.reordered`（§7.19），不改变成员归属，不触发 `taskAdded` / `taskRemoved`。**（v4.3 补充）** 这个顺序不再只是展示顺序——它就是成员被推进的先后顺序，直接决定 §3.3 `taskSegments` 如何切分时间。因此 focus Session 进行中的重排**只允许发生在未来队列内部**：当前成员不得被排到其他位置，未来成员也不得被移动到当前成员之前（见 §3.3 关键规则 14）。已经切分完成、写入已终结 Session 的分段不受任何后续重排影响（Session 是历史事实，见 §3.3 关键规则 11）。
...
10. **合并组是番茄与专注时长的归属单位（v4.3 新增）**。一次由本组触发的 focus Session 正常完成，记 1 个有效番茄、全额 `actualDuration` 计入本组；组内成员任务的有效番茄数不增加，成员只从 `taskSegments` 拿到各自那一段耗时（见 §3.3 关键规则 12）。统计页默认以合并组为条目展示（用 `title` 作为条目名），成员明细需展开该条目才可见；Goal（§3.9）维度的投入时长取成员分段耗时，不取本组的整段时长（见 §8.13）。
11. **成员在组内被勾完成，是 `taskSegments` 切分的事实来源（v4.3 新增）**。产品必须提供"在专注进行中逐个勾选组内成员完成"的入口，否则时间无法按成员切分。勾完成即写 `Task.completed`（§7.1），同时确定该成员分段的结束时刻；具体切分算法见 §3.3 关键规则 13。进行中只允许勾选**当前成员**（判定见 §3.3 关键规则 13）——分段模型是严格顺序的，跳过当前成员去勾选后面的未来成员无法切分时间，必须拒绝。

12. **合并组的成功终态 `'completed'`**。触发时机是一轮合并 focus 正常 `completed`、对应休息流程结束后，用户明确确认整组完成。完成不要求全部成员已完成；`mergeGroup.completed` 必须保存完成时 `finalTaskIds` 与 `incompleteTaskIds` 快照。完成事务必须同时写 Group 终态、`completedAt`、清空全部成员 `Task.mergeGroupId` 并追加 Event，任一步失败整体回滚。

    清空归属只改变“当前所属”，不改变 Task 自身状态：已完成成员保留 `completedAt` / `completionSource`，未完成成员保持 `active`；不自动归档、删除、取消完成，也不追加任何 `task.*` Event。`completed` 与 `dissolved` 互斥；两种终态之后唯一允许的 MergeGroup 写入都是重命名，且改名只改显示名，不改变成员、耗时、番茄数、执行或统计事实。

    历史归属由 `MergeGroup.taskIds`、`Session.mergeGroupId`、`Session.taskSegments` 与 Event 关联共同承载；查询与统计不得依赖已被清空的 `Task.mergeGroupId`。

13. **活动 focus Session 期间的成员锁定（v4.3 新增）**。该组存在 `status='active'` 的 focus Session 时，本轮当前成员被锁定，数据层必须拒绝：把当前成员移出本组（`mergeGroup.taskRemoved`）、把当前成员重排到其他位置、把未来成员排到当前成员之前（`mergeGroup.reordered`）、通过 `mergeGroup.dissolved` 整体解散来间接移出当前成员，以及对当前成员的删除 / 预估调整等普通 Task 修改。未来成员不受锁定，可自由新增、移出与在未来队列内部重排；新加入的成员一律排在当前成员之后。完整规则与"解除锁定的两条路径"见 §3.3 关键规则 14——这条约束必须由每一条相关 command / write path 强制保证，**不得只靠 UI 隐藏按钮**。

14. **移出未来成员会同步缩小已开跑 Session 的名单**。focus Session 进行中把一个尚未轮到的未来成员移出本组后，它不再属于本组，因此也不会出现在本轮 Session 终结时的 `taskIds` 快照里（快照口径见 §3.3 关键规则 11）。快照因此可能只剩 1 个成员，这是合法的（§3.3 字段一致性约束 15）。

15. **成员数量不足不打断正在进行的 Session（v4.3 新增）**。用户在 active focus Session 进行中移出最后一个未来成员，会使 `MergeGroup.taskIds` 的总成员数暂时降到 1。此时：

    - 合并组**保持 `'active'`**，**不**立即解散——关键规则 2 的自动解散推迟到本轮结算，绝不制造"MergeGroup 已 `dissolved`、但 `Task.mergeGroupId` 仍保留"的中间状态；
    - 当前成员在本轮 Session 结束前保持锁定，不能再被移出；
    - 本轮 Session（`completed` 或 `discarded`）原子收尾后再结算：用户确认这一组完成 → `status='completed'`（关键规则 12）；未确认完成且总成员确实仍只剩 1 个 → 按关键规则 2 解散（`dissolvedReason='membersBelowMinimum'`），该 Task 回到独立任务状态。

    这条例外**只**针对"组的总成员数被移空到 1"这一种情形，说的是**已经开跑的一轮不被打断**；它不改变下一条的开轮门槛。

16. **≥ 2 既是建组门槛，也是每一轮的开轮门槛（v4.3.1 改判）**。开启任何一轮合并 focus Session（含追加预估后的续轮）之前，组内**未完成成员必须 ≥ 2**，否则拒绝开轮：只剩一件事要做时，它就该作为独立任务自己跑一个完整番茄，而不是继续挂在合并组里占用合并组的轮次编号。

    > 本条**推翻**了 v4.1 时期"续轮排除已完成成员后只剩 1 个未完成成员也照开"的决定（对应 commit `5559660`）。用户已明确将"开新一轮前必须重新满足至少 2 个未完成成员"定为**通用规则**，不限于关键规则 15 的移空场景。
    >
    > 注意本条与 §3.3 字段一致性约束 15（`mergeGroupId` 非 null 时 `taskIds ≥ 1`）**不矛盾**：约束 15 管的是"什么样的 Session 记录可以存在"，本条管的是"什么时候允许开一轮"。一轮**开跑之后**，成员被移出会让 `Session.taskIds` 合法地缩到 1（关键规则 15），那条记录必须能写下去，所以存储层不能收紧成 ≥ 2；门槛只由开轮命令强制。

**字段一致性约束**

以下跨字段一致性规则必须由数据层强制保证，任何写入操作不得违反：

1. `status ∈ {'active', 'limitReached'}` 时，`taskIds` 长度必须 ≥ 2，`dissolvedAt` / `dissolvedReason` / `completedAt` 必须为 null。**唯一例外（v4.3 新增）**：该组存在 `status='active'` 的 focus Session 时，允许 `taskIds` 长度暂时降到 1（关键规则 15）；本轮结算后必须回到 ≥ 2，或转入 `'completed'` / `'dissolved'`。
2. `status='dissolved'` 时，`dissolvedAt` 必须非 null，`dissolvedReason` 必须非 null，`completedAt` 必须为 null。
...
8. **（v4.3 新增）** `title` 必须为非空字符串，长度 ≤ 200；不允许写入空字符串或纯空白字符串。创建 MergeGroup 时若调用方未提供名称，由实现端写入系统默认名，**不允许**留空。
9. **（v4.3 新增）** `status='completed'` 时，`completedAt` 必须非 null，`taskIds` 长度必须 ≥1，`dissolvedAt` / `dissolvedReason` 必须为 null；`status ≠ 'completed'` 时 `completedAt` 必须为 null。
10. **（v4.3.2 修订）** `status ∈ {'completed', 'dissolved'}` 是终态：除 `mergeGroup.renamed` 外，不允许写入 `mergeGroup.taskAdded` / `taskRemoved` / `reordered` / `estimateAdjusted`，也不允许开启指向该组的新 focus Session。
11. **（v4.3 新增）** 该组存在 `status='active'` 的 focus Session 时，本轮当前成员被锁定（关键规则 13）：任何会把当前成员移出本组、改变其在 `taskIds` 中位置、或把未来成员排到它之前的写入，以及 `mergeGroup.dissolved`，必须被拒绝。
12. **（v4.3.2 新增）** `status ∈ {'completed', 'dissolved'}` 时，不允许存在任何 `Task.mergeGroupId` 指向本组；反向地，`Task.mergeGroupId` 非 null 时只能指向 `active` / `limitReached` 合并组。

实现端在写入或更新 MergeGroup 时，必须验证以上规则，违反规则的写入操作应被拒绝。

---

### 3.9 Goal（中长期主线目标，v4.3 新增）

Goal 表示一个**跨周 / 跨月的中长期目标**，产品语言里称为"主线任务"（如"阅读《人类简史》"、"完成年度体检"）。它是**一层单独的清单**，与 Task 平行，不是 Task 的一种。

Goal 的职责只有一个：**被独立任务向上关联**（`Task.goalId`，见 §3.1），从而回答"我在这件长期目标上，累计投入了多少个任务、多少个番茄、多少时间，推进到哪一步了"。它**本身不是可执行单元**：不能被排进今日待办、不能开始番茄计时、没有预估番茄数、不适用任何"7 个番茄封顶 / 达标后强制拆分"的任务规则。

> **与"今日主线分类"（`DayPlan.mainCategory`，§3.2）不是一回事**。后者是 v4.2 引入的当天默认大类（`work` / `study` / `side` / `life`），用于任务自动打标；本节的 Goal 是用户自建的具体中长期目标，二者名称中都含"主线"二字，但分属不同维度：一个 Goal 自己也带 `category` 字段。规范正文与实现端一律用 `Goal` / `goalId` 指代本实体，不使用"主线任务"作为字段名或类型名，以免与 `mainCategory` 混淆。
...
|---|---|---|---|---|
| `id` | `string`（UUID v7） | 否 | 写入时生成 | 实体唯一标识；取值约束：UUID v7 格式 |
| `title` | `string` | 否 | 无 | 目标标题；取值约束：非空字符串（不允许空字符串或纯空白），最大长度 200 字符（与 §3.1 Task.title 一致） |
| `note` | `string \| null` | 是 | `null` | 目标备注，用于记录这个目标的背景、验收标准等；取值约束：无特殊约束 |
| `category` | `string \| null`（枚举） | 是 | `null` | 目标大类标签，枚举与 §3.1 `Task.category` 完全一致（`'work'` / `'study'` / `'side'` / `'life'`）；`null` = 未分类；**与其名下 Task 的 `category` 相互独立，不校验、不联动**（见 §3.1 关键规则 14、本节关键规则 7）；取值约束：取值为 `null` / `'work'` / `'study'` / `'side'` / `'life'` 之一 |
| `status` | `string`（枚举） | 否 | `'active'` | 目标状态，枚举值见下方；取值约束：必须取自枚举值之一（active / completed / archived / deleted） |
| `outcome` | `string \| null` | 是 | `null` | 归档结果；`null` = 未归档；`'completed'` = 达成归档；`'abandoned'` = 放弃归档（目标不再推进，但历史投入保留可查）；取值约束：取值为 null / 'completed' / 'abandoned' 之一 |
| `sortIndex` | `number` | 否 | 当前列表最大 sortIndex + 1000；列表为空时为 1000 | 在计划页主线目标清单中的排序索引；步长 1000 便于拖拽时取中间值插入，避免全表重排（与 §3.1 Task.sortIndex 同款策略）；取值约束：整数，≥ 0 |
| `metadata` | `object` | 否 | `{}` | 附加元数据对象，永远不为 null（可为空对象），内部字段均可选，见下方；取值约束：必须为非 null 对象，内部字段均为可选 |
| `createdAt` | `string` | 否 | 写入时生成 | 记录首次写入时间；取值约束：ISO 8601 带时区格式，不允许缺省时区 |
| `updatedAt` | `string` | 否 | 写入时生成 | 记录最近修改时间；取值约束：ISO 8601 带时区格式，不允许缺省时区 |
| `completedAt` | `string \| null` | 是 | `null` | 目标完成时间；未完成时为 null；目标被取消完成时改回 null，历史完成轨迹通过 Event 追溯，Goal 本体只反映当前状态；取值约束：ISO 8601 带时区格式或 null |
| `archivedAt` | `string \| null` | 是 | `null` | 归档时间（达成归档或放弃归档时写入）；取值约束：ISO 8601 带时区格式或 null |
| `deletedAt` | `string \| null` | 是 | `null` | 软删除时间戳（同时用作 §2.3 同步 tombstone）；`null` = 未删除；取值约束：ISO 8601 带时区格式或 null |
| `deletedReason` | `string \| null` | 是 | `null` | 删除原因，可选；取值约束：`null` / `'userDeleted'` / `'dataCleanup'` 之一，不允许写入任意自由文本；`status='deleted'` 时 `deletedAt` 必须非 null，`deletedReason` 可为 null，但若非 null 必须符合上述枚举 |
| `schemaVersion` | `number` | 否 | 写入时取当前 schema 版本 | 该条记录写入时的 schema 版本号（见 §2.3）；取值约束：正整数，≥ 1 |
| `deviceId` | `string \| null` | 是 | `null` | （可选预留）写入设备标识（见 §2.3，Phase 5+ 启用）；取值约束：null 或非空字符串 |
| `syncedAt` | `string \| null` | 是 | `null` | （可选预留）最近一次同步成功时间（见 §2.3，Phase 5+ 启用）；取值约束：ISO 8601 带时区格式或 null |
...
| 值 | 含义 |
|---|---|
| `'active'` | 推进中，在计划页主线目标清单中展示，可被任务关联 |
| `'completed'` | 已达成，已写入 `completedAt`；不再出现在计划页主线清单的默认视图中，统计与归档视图仍可查 |
| `'archived'` | 已归档（达成归档或放弃归档），`outcome` 字段记录归档类型；归档视图中可展开查看其名下全部任务 |
| `'deleted'` | 软删除，`deletedAt` 非 null，正常查询中不显示；其名下 Task 的 `goalId` **不清空**（见关键规则 6） |

**metadata 对象结构**

```
{
  tags?:  string[]   标签数组，可选；与 §3.1 Task.metadata.tags 是同一套自由标签维度，
                     Goal 与 Task 都可以打标签，互不影响、不要求一致
  color?: string     目标颜色标记（如 "#FF6B6B"），可选，供计划页清单区分展示使用
}
```

**关键规则**

1. **Goal 不是可执行单元，永远不直接参与计时**。任何 `Session`（§3.3）都不允许关联 Goal——`Session.taskIds` 只装 Task id，不存在 `Session.goalId` 这样的字段。用户想推进一个 Goal，只能给它挂上具体的 Task，然后对 Task 计时。
2. **Goal 没有预估**。本实体不设 `estimatedPomodoros` / `estimateRounds` 字段，也**不适用** §3.1 关键规则 9 / 10 的"单个 Task 最多 7 个番茄、达标后强制拆分"约束——中长期目标本来就会跨越远超 7 个番茄的投入量，对它做番茄封顶没有意义。
3. **进度与投入统计不存在 Goal 上**，一律从其名下 Task 及对应 Session / Event 派生，口径见 §8.13。Goal 本体不存"已投入番茄数""已完成任务数""进度百分比"等任何冗余统计字段（与 §3.1 关键规则 1 同一原则）。
4. **Goal 与 Task 是一对多**：一个 Goal 可以关联任意多个 Task，一个 Task 最多只能关联一个 Goal（见 §3.1 `goalId` 字段定义）。同一个 Task 不能同时算在两个 Goal 头上——若实际出现这种需要，属于 Goal 划分不够清晰，由用户在产品层面重新拆分目标解决，数据层不提供多关联能力。
5. **Goal 之间没有层级**，不允许嵌套（不存在 `Goal.parentGoalId` 这样的字段）。v4.3 废弃 Task 层级的同时，不在 Goal 上重新引入层级结构。
6. **删除 Goal 一律软删除，且不清空名下 Task 的 `goalId`**。写 `deletedAt`、`status` 改为 `'deleted'`，其名下所有 Task 的 `goalId` **保持不变**，继续指向这条 tombstone 记录——这样误删可以直接恢复，历史归属不会被一次删除操作抹掉。查询层默认过滤 `deletedAt != null` 的 Goal；统计层把 `goalId` 指向已删除 Goal 的任务归入独立的"已删除目标"分组，不混入未分类，也不在主线清单中展示（见 §8.13）。**因为该操作影响面覆盖其名下全部历史任务，产品层必须以弹窗二次确认后才允许执行**（本条为数据规范对实现端提出的强制交互要求，不是可选建议）。
7. **`Goal.category` 与其名下 `Task.category` 相互独立**，不校验、不联动、不追溯覆盖：给 Task 关联 Goal 不改写 Task 已有的 `category`，修改 Goal 的 `category` 也不追溯改写名下任何 Task（见 §3.1 关键规则 14）。
8. **Goal 的完成与其名下任务的完成状态不做任何自动联动**。名下 Task 全部完成，**不**自动把 Goal 置为 `'completed'`（用户可能还想继续往这个目标里加新任务）；把 Goal 标记为完成，也**不**自动完成 / 归档其名下尚未完成的 Task。两个方向都由用户显式操作，数据层不代为推断。
9. **Goal 不进 DayPlan**。`DayPlan.taskIds`（§3.2）只装 Task id，任何情况下都不允许写入 Goal id。"今天要推进哪个目标"这件事，通过把该 Goal 名下的具体 Task 加入今日待办来表达。
10. **Goal 不物理删除**，遵循 §2.4 软删除规则，保留 tombstone 供同步与回溯。

**字段一致性约束**

以下跨字段一致性规则必须由数据层强制保证，任何写入操作不得违反：

1. `status='completed'` 时，`completedAt` 必须非 null。
2. `status='archived'` 时，`archivedAt` 必须非 null，`outcome` 必须非 null（`'completed'` 或 `'abandoned'`）。
3. `status='deleted'` 时，`deletedAt` 必须非 null。
4. `outcome` 非 null 时，`status` 必须为 `'archived'`。达成归档（`outcome='completed'`）时，若该 Goal 此前已完成，原 `completedAt` 必须保留、不得清空；放弃归档（`outcome='abandoned'`）**不强制要求** `completedAt` 非 null。
5. 目标被取消完成时，`completedAt` 必须改回 null，`status` 改为 `'active'`；该操作只适用于尚未归档的 `'completed'` 状态，已归档的 Goal 不通过此路径恢复（与 §3.1 一致性约束 7 同款处理）。
6. `title` 必须为非空字符串，长度 ≤ 200；不允许写入空字符串或纯空白字符串。
7. `category` 非 null 时，取值必须为 `'work'` / `'study'` / `'side'` / `'life'` 之一。**注意与 §3.1 Task 的差异**：Goal **不设** `categorySource` 字段——Goal 的分类只能由用户手动设置，不存在自动打标路径（§7.1 `task.categoryChanged` 的自动判定只作用于 Task），因此没有"来源"需要区分。

实现端在写入或更新 Goal 时，必须验证以上规则，违反规则的写入操作应被拒绝。
...
| Domain | 含义 | §7 对应节 | 备注 |
|---|---|---|---|
| `task` | 任务本体变化，以及任务排序、跨列表移动与 Goal 归属操作 | §7.1 / §7.4 | 排序、移动与 Goal 关联事件（`task.reordered` / `task.goalLinked` 等）归入 `task` 域；§7.4 单列为独立分节，方便查阅，不另立 domain 前缀 |
| ~~`subtask`~~ | ~~子任务特有行为~~ | ~~§7.2~~ | **v4.3 已废除**：子任务机制（`Task.parentId`）整体废弃，本 domain 及其全部事件类型一并移除，不保留兼容，见 §3.1「关于 `parentId` 的废弃说明」；`subtask` 不得再作为 domain 前缀使用 |
| `goal` | 中长期主线目标（Goal）本体的创建、修改、完成、归档、排序与删除 | §7.20 | v4.3 新增；对应 §3.9 Goal 实体。注意：任务与 Goal 之间的**关联 / 解除关联**不属于本域，归 `task` 域（`task.goalLinked` / `task.goalUnlinked`，见 §7.4）——变的是 Task 的字段，不是 Goal 的字段 |
...
| `skipped` | 跳过（break）|
| `started` | 开始计时 |
| `added` | 添加到集合（任务加入 DayPlan、任务加入合并组）|
| `removed` | 从集合移除（任务从 DayPlan 移除）|
| `reordered` | 在同一列表内调整排列顺序 |
| `linked` | 建立到另一实体的归属关联（任务关联到某个 Goal）|
| `unlinked` | 解除已建立的归属关联（任务脱离原 Goal）|
| `renamed` | 重命名（合并组改名）|
...
2. **`task` 与 `goal` 不混用（v4.3 改写）** → `goal.` 前缀只用于 Goal 实体本身的字段与生命周期变化（改名、改分类、完成、归档、删除等）；任务与 Goal 之间建立或解除关联，改的是 `Task.goalId` 这个 Task 字段，一律用 `task.` 前缀（`task.goalLinked` / `task.goalUnlinked`），不得写成 `goal.taskAdded` 之类。原 v4.0–v4.2 的 `task` 与 `subtask` 不混用规则随子任务机制废除一并删除。
...
5. **`task.positionChanged` 已废弃** → 不允许在 v4 中使用此事件。一次拖拽操作根据语义分别写 `task.reordered`（同列表内重排）、`task.movedToToday`（加入今日）、`task.movedToList`（移回清单）等具体事件。若单次操作同时产生多条事件，通过 `correlationId` 关联。**（v4.3 修订）** 原列举的 `task.reparented`（父任务变化）已随子任务机制废除而删除，不得再写入；把任务拖进某个 Goal 的操作写 `task.goalLinked`（§7.4）。
...
本节定义 Task 生命周期相关事件。任务排序 / 跨列表移动 / Goal 归属变化（重排、加入今日、移回清单、关联或解除关联主线目标）见 §7.4；Goal 实体自身的变化见 §7.20；DayPlan 侧变更见 §7.3。（原 §7.2 子任务事件域已于 v4.3 整节废除。）
...
**payload**：`{ title, goalId, estimatedPomodoros, source }`

`goalId`（v4.3 新增，取代原 `parentId`）：创建时即指定的所属中长期主线目标（Goal，§3.9）id；未指定时为 `null`。绝大多数创建路径写 `null`（用户先建任务、之后再关联 Goal，走 §7.4 `task.goalLinked`）；只有在用户从某个 Goal 的详情 / 卡片里直接新建任务这类入口下才在创建时即带值。取值约束：`null` 或合法的 Goal UUID v7。**创建时即带 `goalId` 的场景不额外触发 `task.goalLinked`**——归属已由本事件的 payload 表达，补发关联事件会造成同一事实被记两遍。
...
**不应触发**：任务已存在后的任何字段修改（→ `task.updated` 或对应专属事件）；任务被移入今日待办（→ `dayPlan.taskAdded`）；用户输入 `estimatedPomodoros > 7`（→ 拒绝写入，触发拆分提示）；已存在的任务被关联到某个 Goal（→ §7.4 `task.goalLinked`，不是新建）；新建一个中长期主线目标（→ §7.20 `goal.created`，Goal 不是 Task）。
...
**顶层关联字段**：`taskId`；`completionSource='pomodoro'` 时可填写 `sessionId`（触发完成确认的 focus session）；任务在计时页 session 过程中被手动勾选完成时，也可填写 `sessionId` 记录计时上下文——此时 `completionSource` 仍为 `'manual'`，`sessionId` 仅表示"发生在哪个 session 期间"，不改变完成方式的语义。合并番茄钟场景下，组内成员被逐个勾选完成时同样填写 `sessionId`（该合并 focus Session 的 id），该时刻同时决定该成员在 `Session.taskSegments` 中的分段结束时间（见 §3.3 关键规则 13）。
...
> **合并番茄钟场景下的取值（v4.3 已拍板，原 §14【V43-1】【V43-2】结案）**：成员任务在合并番茄进行中轮到自己并被勾完成时：
>
> - `completionSource` 记 **`'pomodoro'`**。这里的 `'pomodoro'` 表示"任务是在番茄专注流程中完成的"，**不表示该成员拥有一个完整有效番茄**；要判断它是不是在合并番茄里完成的，通过关联的 `sessionId`（其 Session 的 `mergeGroupId` 非 null）判断。
> - `validFocusCountAtCompletion` 记 **`0`**。合并组成员不获得完整有效番茄（§3.3 关键规则 12），**不得**写小数，也**不得**把成员的分段时长折算成番茄数。因为合并资格要求成员此前从未有过任何 focus Session（§3.8 关键规则 9），这类成员的 Task 有效番茄历史本来就是 0，该取值与 §8.5.1 的定义自洽，不是特例填空。
>
> 因此以下组合是**合法且预期**的，实现与统计都必须接受：`completionSource='pomodoro'` + `validFocusCountAtCompletion=0` + 成员实际投入时间 > 0。
>
> **下游统计必须据此审计**：**不得**再把 `completionSource='pomodoro'` 自动等同于"该 Task 至少拥有一个有效番茄"。特别地，合并组成员任务必须**排除**出 §8.5.3 基于 Task 有效番茄数的预估准确率 / 预估偏差样本——否则会把"预估 1、有效番茄 0"错误解释成预估偏大。成员的投入用实际时间（`Session.taskSegments` 的分段耗时）统计；如果将来需要时间口径的预估准确率，另行设计，**不得**自行新增快照字段。
...
**不应触发**：Goal（中长期主线目标）被标记完成（→ §7.20 `goal.completed`；Goal 不是 Task，也不因名下任务全部完成而自动完成，见 §3.9 关键规则 8）；任务归档（→ `task.archived`）；拆分归档（→ `task.split` + `task.archived`）。
...
**不应触发**：任务归档（→ `task.archived`）；从今日待办移回活动清单（→ §7.4 / `dayPlan.taskRemoved`，不修改 Task 字段）；关联到某个 Goal 的任务被软删除（同样只触发本事件，`goalId` 保持不变、不额外触发 §7.4 `task.goalUnlinked`，见 §7.4）；Goal 本身被软删除（→ §7.20 `goal.deleted`）。
...
### 7.2 ~~Subtask（子任务）~~ —— v4.3 已整节废除

**本节在 v4.3 被整节废除，不再定义任何事件。**

子任务机制（`Task.parentId`）连同本节原有的 4 个事件——`subtask.added`、`subtask.reordered`、`subtask.reparented`、`subtask.unparented`——已随 §3.1 `parentId` 字段一并从规范中移除，理由与废弃范围见 §3.1「关于 `parentId` 的废弃说明」。

**对实现端的要求**：

1. `subtask.*` 不再是合法的 Event `type`。按 §3.4 一致性约束，写入未定义 type 必须被拒绝——实现端应从 EventType 枚举中**删除**这 4 个值，而不是保留为 deprecated。
2. 废除时产品尚未正式投入使用，无历史 Event 需要兼容，因此不保留读取侧的兼容分支。
3. 原先由子任务表达的"一件大事下面挂着若干小事"，v4.3 起一律用 Goal（§3.9）+ `Task.goalId` 表达；对应事件见 §7.20（Goal 本体）与 §7.4（任务与 Goal 的关联 / 解除关联）。
4. 本节号 `7.2` **保留占位、不回收**，以免后续章节整体重编号导致其他文档中的 §7.x 引用失效。
...
### 7.4 Task 排序、跨列表移动与 Goal 归属

本节定义活动清单内排序、活动清单与今日待办之间的移动、以及任务与中长期主线目标（Goal，§3.9）之间建立 / 解除归属关联的事件。所有事件使用 `task.` 前缀。

**Domain 级说明**：
- **（v4.3 修订）** 任务层级变化事件（`task.reparented`）已随子任务机制废除而删除，本节不再定义；§7.2 `subtask.*` 整节同步废除。取而代之的是本节的 `task.goalLinked` / `task.goalUnlinked`。
- Goal 实体自身的创建、改名、完成、归档、删除等 → §7.20 `goal.*`，不在本节定义。本节两个事件改的是 `Task.goalId` 这个 Task 字段，因此用 `task.` 前缀（见 §6.4 禁止混用规则第 2 条）。
...
**不应触发**：今日待办列表内重排（→ `dayPlan.taskReordered`）；合并组内成员重排（→ §7.19 `mergeGroup.reordered`）；任务跨列表移动（→ `task.movedToToday` / `task.movedToList`）；计划页主线目标清单内重排（→ §7.20 `goal.reordered`，重排的是 Goal 不是 Task）。

---

#### task.goalLinked（P2，v4.3 新增）

**顶层关联字段**：`taskId`、`goalId`（关联到的目标）。

**payload**：`{ fromGoalId, toGoalId, source }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `fromGoalId` | `string \| null` | 是 | 关联前的 `Task.goalId`；`null` = 此前未关联任何 Goal；非 null = 本次是从旧目标**改绑**到新目标（改绑只写本事件一条，不拆成先 unlink 再 link）；取值约束：null 或合法的 Goal UUID v7 |
| `toGoalId` | `string` | 否 | 关联后的 `Task.goalId`；取值约束：合法的 Goal UUID v7，且必须与 `fromGoalId` 不同（相同不写入本事件） |
| `source` | `string`（枚举） | 否 | 本次关联的操作来源；取值约束：`'single'`（单个任务逐条关联）/ `'batch'`（批量选中多个任务一次性关联）之一 |

**说明**：用户把一个已存在的任务归到某个中长期主线目标名下时触发，操作完成后该 Task 的 `goalId` 变为 `toGoalId`。

**关于关联时机**：本事件**不限制任务处于什么状态**——活动清单中的任务、今日待办中的任务、乃至**已完成或已归档的任务**都可以被关联或改绑。因为 Goal 维度的统计是按"任务当前归属"派生的（见 §8.13），把一个上个月完成的任务补绑到某个 Goal，该 Goal 的累计投入会立即把这条历史投入计入——**这是预期行为，不是数据异常**；本事件即为该次归属变更留痕，使数字变化可追溯。

**关于批量关联**：批量关联不设独立事件，按选中的每个任务各写一条本事件，`source='batch'`，同一次批量操作的多条事件共享同一个 `correlationId`。批量执行遵循既有批量约定：整批先做零写入预检，随后逐个任务独立原子提交（实体变更 + 本事件同事务，见 §3.4）。

**典型触发**：用户在活动清单里把某个任务拖进"阅读《人类简史》"这个主线目标；用户多选 5 个任务后一次性指定归属目标（`source='batch'`）；用户把某任务从"目标 A"改绑到"目标 B"（`fromGoalId` 非 null）。

**不应触发**：创建任务时即指定 `goalId`（→ 由 `task.created` 的 payload 表达，不补发本事件，见 §7.1）；解除关联、变为不属于任何目标（→ `task.goalUnlinked`）；`toGoalId` 与 `fromGoalId` 相同（未实际变化不触发）；Goal 自身被创建或改名（→ §7.20 `goal.*`）；任务被加入合并组（→ §7.19 `mergeGroup.taskAdded`；合并组与 Goal 是互不影响的两个维度，见 §3.1 关键规则 12）。

---

#### task.goalUnlinked（P2，v4.3 新增）

**顶层关联字段**：`taskId`、`goalId`（解除关联前所属的目标）。

**payload**：`{ previousGoalId, source }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `previousGoalId` | `string` | 否 | 解除关联前的 `Task.goalId`；取值约束：合法的 Goal UUID v7 |
| `source` | `string`（枚举） | 否 | 本次解除关联的操作来源；取值约束：`'single'` / `'batch'` 之一，语义同 `task.goalLinked` |

**说明**：用户把任务从某个中长期主线目标下摘出、使其重新成为不隶属任何目标的独立任务时触发。操作完成后该 Task 的 `goalId` 变为 `null`。与 `task.goalLinked` 一样，不限制任务当前状态，已完成 / 已归档的任务同样可以解除关联。

**不应触发**：改绑到另一个目标（→ `task.goalLinked`，`fromGoalId` 非 null；改绑只写一条事件，**不得**拆成 `goalUnlinked` + `goalLinked` 两条）；所属 Goal 被软删除（→ §7.20 `goal.deleted`；**删除 Goal 不清空名下任务的 `goalId`**，因此不触发本事件，见 §3.9 关键规则 6）；任务被软删除（→ `task.deleted`，`goalId` 保持不变）。
...
- **合并番茄钟场景**（以 `Session.mergeGroupId` 非 null 判断，见 §3.3、§3.8）：`focus.started` / `focus.completed` / `focus.discarded` 按 `taskIds` 数组的每个成员各写一条独立事件，共享同一个 `sessionId` / `mergeGroupId` / `correlationId`。已开跑后名单缩到 1 仍是合并场景，不能仅用 `taskIds.length ≥2` 判断。
...
**顶层关联字段**：`sessionId`（正在进行的 focus Session id）、`taskId`（关联任务）；若对应 `Session.dayPlanId` 非 null，则同步填写 `dayPlanId`。**合并番茄钟场景**（以 `Session.mergeGroupId` 非 null 判断）：打扰不特定针对某一个成员任务，`taskId` 固定为 null，只通过 `sessionId` 关联，需要时反查该 Session 的成员快照；不按成员数量重复触发本事件。
...
本节定义合并番茄钟（MergeGroup，见 §3.8）生命周期相关事件：创建、成员增减、成员排序、预估追加、重命名、完成、解散。

**Domain 级说明**：

- 合并组触发的 focus Session 本身（开始 / 完成 / 作废）仍归 §7.5 `focus.*` 域，不在本节重复定义；合并场景下 `focus.started` / `focus.completed` 按参与任务数量各发一条，见 §7.5 补充说明。
- 本节共 **8 个**事件（v4.1 最初 6 个：`created` / `taskAdded` / `taskRemoved` / `reordered` / `estimateAdjusted` / `dissolved`；v4.3 新增 `renamed` 与 `completed`），所属功能批次为"合并番茄钟"，是 Phase 1–3 封板之后新增的独立功能批次，不套用 §10 既有 P1–P5 编号（避免与既定 Phase 范围混淆），Phase 归属见 §10.7。文档其他位置若出现"5 个 / 6 个 mergeGroup.\* 事件"的旧计数，一律以本节实际定义为准。
- **活动 Session 期间的写入限制**：该组存在 `status='active'` 的 focus Session 时，本轮当前成员被锁定（§3.8 关键规则 13、§3.3 关键规则 14）。`taskRemoved` / `reordered` 若会移出当前成员、改变其位置、或把未来成员排到它之前，必须**拒绝写入、不产生事件**；`dissolved` 在此期间一律拒绝（不得用整体解散绕过锁定）。`taskAdded` 允许，但新成员只能排在当前成员之后。
- **终态限制**：`status ∈ {'completed', 'dissolved'}` 时，只允许 `mergeGroup.renamed`；`taskAdded` / `taskRemoved` / `reordered` / `estimateAdjusted` 一律拒绝。终态改名只改变显示名，不改变任何成员、执行或统计事实，见 §3.8 字段一致性约束 10。
...
**payload**：`{ title, taskIds, estimatedPomodoros }`

| 字段 | 类型 | 说明 |
|---|---|---|
| `title` | `string` | **（v4.3 新增）** 创建时写入的合并组名称，见 §3.8 `title` 字段；调用方未提供时由实现端写入系统默认名，不允许为空 |
...
**典型触发**：4 个任务合并计时，第一个番茄到点还剩 2 个没做完，用户选"追加预估番茄"，`estimatedPomodoros` 从 1 变为 2；下一轮开轮前再次校验未完成成员仍至少 2 个，满足后才开启第 2 个 focus Session，已完成成员继续作为历史名单留在组内。
...
#### mergeGroup.renamed（合并番茄钟功能批次，v4.3 新增）
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `oldTitle` | `string` | 改名前的 `MergeGroup.title`；取值约束：非空字符串，长度 ≤ 200 |
| `newTitle` | `string` | 改名后的 `MergeGroup.title`；取值约束：非空字符串，长度 ≤ 200，且必须与 `oldTitle` 不同（相同不写入本事件） |

**说明**：用户给这个"杂事番茄"改名时触发。v4.3 起合并组本身是番茄与专注时长的归属单位、会作为独立条目出现在统计页（见 §3.3 关键规则 12、§8.5），因此需要一个用户可读、可自定义的名字来区分不同的合并组；本事件为该名称的变更留痕。改名不影响成员归属、预估、状态与任何统计数值。

**典型触发**：用户把默认名"杂事番茄"改成"周一杂事清理"；用户在统计页历史记录里发现两个同名合并组，回头给其中一个改名以便区分。

**不应触发**：创建合并组时写入初始名称（→ `mergeGroup.created` 的 payload 已携带 `title`，不补发本事件）；`newTitle` 与 `oldTitle` 相同（未实际变化不触发）；组内成员任务改标题（→ §7.1 `task.updated`，改的是 Task.title 不是 MergeGroup.title）。

---

#### mergeGroup.completed（合并番茄钟功能批次，v4.3 新增）

**顶层关联字段**：`mergeGroupId`、`sessionId`（触发完成确认的刚收尾合并 focus Session）。

**payload**：`{ completedAt, validFocusCountAtCompletion, finalTaskIds, incompleteTaskIds }`

| 字段 | 类型 | 说明 |
|---|---|---|
| `completedAt` | `string` | 用户确认这一组完成的时刻，与写入的 `MergeGroup.completedAt` 一致；取值约束：ISO 8601 带时区格式 |
| `validFocusCountAtCompletion` | `number` | 完成那一刻该 MergeGroup 已累计的有效标准 focus 数（`type='focus'`、`status='completed'`、`mergeGroupId` 指向本组、`deletedAt` 为 null）；语义与 §7.1 `task.completed` 的同名字段完全对应，只是归属单位从 Task 换成 MergeGroup；取值约束：整数，≥ 0 |
| `finalTaskIds` | `string[]` | 完成时 `MergeGroup.taskIds` 的完整成员快照；不得重复。开轮时至少 2 个未完成成员，但已开跑 Session 可合法缩到 1，因此本终结快照长度允许 ≥1，不得机械收紧为 ≥2 |
| `incompleteTaskIds` | `string[]` | 完成时仍未完成成员快照；必须是 `finalTaskIds` 子集，可为空，不得重复 |

**说明**：一轮 focus 正常 `completed`、其对应休息流程也已结束后，用户明确确认整组完成时触发。以下四项必须在同一原子事务内完成，任一步失败整体回滚：写 `status='completed'`、写 `completedAt`、清空全部成员 `Task.mergeGroupId`、追加本事件。

清空当前归属不改变成员 Task 状态：已完成成员继续保持 `completed` 并保留完成事实；未完成成员继续保持 `active`，恢复为独立任务；不自动归档、删除、取消完成，也不追加任何 `task.*` Event。用户可以在仍有未完成成员时确认整组完成；`incompleteTaskIds` 不要求为空。

两份成员快照记录的是完成当时的事实，不能在成员日后继续变化后重算。历史归属查询使用 `MergeGroup.taskIds`、`Session.mergeGroupId`、`Session.taskSegments` 与 Event 关联，不依赖已清空的 `Task.mergeGroupId`。

`validFocusCountAtCompletion` 是快照值，写入后不随后续数据修复 / 恢复反向改变，供 §8.5.3 的预估准确率 / 预估偏差直接复用**独立 Task 的既有算法**（同一套三分法，只是把 Task 的 `estimateRounds` 换成 MergeGroup 的 `estimateRounds`），不另写一套算法。

`'completed'` 是不可逆终态：本事件之后不允许再写入 `taskAdded` / `taskRemoved` / `reordered` / `estimateAdjusted`，也不允许开启新的 focus Session；唯一例外是 `renamed`，只改显示名（见 §3.8 关键规则 12、字段一致性约束 10）。

**典型触发**：3 件杂事一轮到点后全部做完，用户确认整组完成；或 4 件完成了 3 件，用户决定剩余 1 件之后独立处理，确认整组完成并把该成员写入 `incompleteTaskIds`。

**不应触发**：组内某个成员任务被勾完成（→ §7.1 `task.completed`，改的是成员不是组）；到点选"追加预估番茄"（→ `mergeGroup.estimateAdjusted`）；到点选"结束"移出未完成成员（→ `mergeGroup.taskRemoved`，可能连带 `mergeGroup.dissolved`）；用户主动整体拆散合并卡片（→ `mergeGroup.dissolved`，`dissolvedReason='manualDissolved'`，语义是取消合并而非完成）。
...
**不应触发**：到点选"结束"但移出未完成成员后剩余成员仍 ≥ 2（→ 只触发 `mergeGroup.taskRemoved`，组不解散，见 §3.8 关键规则 4）；用户完成了组内某个任务但组继续进行（不触发任何 mergeGroup 事件，见 §3.8 关键规则 3）；到点选"追加预估番茄"（→ `mergeGroup.estimateAdjusted`，组不解散）；用户确认成功完成（→ `mergeGroup.completed`，与本事件互斥）。

---

### 7.20 Goal（中长期主线目标，v4.3 新增）

本节定义中长期主线目标（Goal，见 §3.9）本体生命周期相关事件：创建、字段修改、分类变更、完成 / 取消完成、归档、排序、软删除与恢复。

**Domain 级说明**：

- **任务与 Goal 之间建立 / 解除关联不在本节**：那改的是 `Task.goalId` 这个 Task 字段，归 §7.4 `task.goalLinked` / `task.goalUnlinked`（见 §6.4 禁止混用规则第 2 条）。本节所有事件改的都是 Goal 自身的字段。
- **Goal 永远不产生计时类事件**：Goal 不可被排进今日待办、不可开始番茄（见 §3.9 关键规则 1、9），因此不存在 `goal.started` / `goal.focusCompleted` 之类的事件，也不会出现在任何 `Session` 记录里。
- **Goal 的完成与其名下任务的完成状态互不联动**（见 §3.9 关键规则 8）：名下任务全部完成不自动触发 `goal.completed`；`goal.completed` 也不自动完成或归档名下任何任务、不触发任何 `task.*` 事件。
- 本节事件所属功能批次为"中长期主线目标"，是 Phase 1–3 封板之后新增的独立功能批次，不套用 §10 既有 P1–P5 编号，Phase 归属见 §10.8。

---

#### goal.created（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ title, category, note }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `title` | `string` | 否 | 创建时写入的目标标题；取值约束：非空字符串，长度 ≤ 200 |
| `category` | `string \| null` | 是 | 创建时指定的大类；`null` = 创建时未分类；取值约束：`null` / `'work'` / `'study'` / `'side'` / `'life'` 之一 |
| `note` | `string \| null` | 是 | 创建时写入的备注；未填写为 `null`；取值约束：无特殊约束 |

**说明**：Goal 实体首次写入存储时触发。新建 Goal 时其名下没有任何任务，`sortIndex` 按 §3.9 字段默认值取当前清单最大值 + 1000。

**典型触发**：用户在计划页主线目标模块点击新建，输入"阅读《人类简史》"并确认。

**不应触发**：新建普通任务（→ §7.1 `task.created`，Task 与 Goal 是两个实体）；已有任务被关联到某个 Goal（→ §7.4 `task.goalLinked`）；Goal 创建后的任何字段修改（→ `goal.updated` 或对应专属事件）。

---

#### goal.updated（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ changedFields, title, note }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `changedFields` | `string[]` | 否 | 本次实际发生变化的字段名列表；取值约束：非空数组，元素取自 `'title'` / `'note'` |
| `title` | `string \| null` | 是 | 修改后的标题；本次未改标题时为 `null`；取值约束：`null`，或非空字符串且长度 ≤ 200 |
| `note` | `string \| null` | 是 | 修改后的备注；本次未改备注时为 `null`；取值约束：无特殊约束 |

**说明**：Goal 的标题或备注被修改时触发。分类变更不走本事件（→ `goal.categoryChanged`），排序、完成、归档、删除同样各有专属事件。

**典型触发**：用户把目标标题从"读书"改成"阅读《人类简史》"；用户在目标备注里补充验收标准。

**不应触发**：分类变更（→ `goal.categoryChanged`）；清单内拖拽排序（→ `goal.reordered`）；标记完成 / 取消完成（→ `goal.completed` / `goal.uncompleted`）；归档（→ `goal.archived`）；软删除（→ `goal.deleted`）；名下任务的任何变化（→ 对应 `task.*` 事件，Goal 本体未变化时不触发本事件）；提交的新值与原值完全相同（未实际变化不触发）。

---

#### goal.categoryChanged（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ previousCategory, newCategory }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `previousCategory` | `string \| null` | 是 | 变更前的 `Goal.category`；取值约束：`null` / `'work'` / `'study'` / `'side'` / `'life'` 之一 |
| `newCategory` | `string \| null` | 是 | 变更后的 `Goal.category`；`null` 表示用户把它重新清空为未分类；取值约束：同上，且必须与 `previousCategory` 不同 |

**说明**：用户修改某个 Goal 的大类时触发。**与 §7.1 `task.categoryChanged` 有两处关键差异**：（1）Goal 的分类**只有手动路径，没有自动打标**——§7.1 的时间窗口自动判定只作用于 Task，因此 Goal 不设 `categorySource` 字段，本事件 payload 也没有 `source`；（2）修改 Goal 的分类**不追溯、不联动改写其名下任何 Task 的 `category`**（见 §3.9 关键规则 7、§3.1 关键规则 14），两者是各自独立的取值。

**典型触发**：用户把"考取驾照"这个目标从"生活"改成"学习"。

**不应触发**：Task 的分类变更（→ §7.1 `task.categoryChanged`）；创建 Goal 时即指定分类（→ 由 `goal.created` 的 payload 表达，不补发本事件）；`newCategory` 与 `previousCategory` 相同（未实际变化不触发）。

---

#### goal.completed（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ completedAt, linkedTaskCount, completedTaskCount }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `completedAt` | `string` | 否 | 目标完成时刻；取值约束：ISO 8601 带时区格式 |
| `linkedTaskCount` | `number` | 否 | 完成那一刻该 Goal 名下任务总数（快照值，含已完成与未完成，不含已软删除的任务）；取值约束：整数，≥ 0 |
| `completedTaskCount` | `number` | 否 | 完成那一刻该 Goal 名下已完成任务数（快照值）；取值约束：整数，≥ 0，且 ≤ `linkedTaskCount` |

**说明**：用户把一个中长期目标标记为已达成时触发，`Goal.status` 变更为 `'completed'`、`completedAt` 写入。两个计数是**快照值**，用于事后回溯"达成这个目标时，手头拆了多少任务、完成了多少"——之所以存快照而不是每次查询重算，是因为用户在目标完成后仍可能继续关联 / 解除关联任务（见 §7.4），重算会让历史数字漂移。快照值不写入 Goal 本体（Goal 不存储衍生统计字段，见 §3.9 关键规则 3）。

**允许"未全部完成即达成"**：`completedTaskCount` 可以小于 `linkedTaskCount`，数据层不校验两者相等，也不阻止用户在名下仍有未完成任务时把目标标记为完成——中长期目标的达成标准由用户自己判断，不由任务勾选情况推断（见 §3.9 关键规则 8）。

**典型触发**：用户读完《人类简史》，在计划页把该主线目标勾选为已完成。

**不应触发**：名下任务全部完成（**不自动触发本事件**，见 §3.9 关键规则 8）；普通任务完成（→ §7.1 `task.completed`）；目标归档（→ `goal.archived`；完成与归档是两步，完成后可继续停留在完成态，不强制立即归档）。

---

#### goal.uncompleted（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ previousCompletedAt }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `previousCompletedAt` | `string` | 否 | 取消完成前该 Goal 的 `completedAt` 值；取值约束：ISO 8601 带时区格式 |

**说明**：用户撤销"已达成"标记时触发，`Goal.status` 改回 `'active'`、`completedAt` 改回 `null`。按 §3.4 关键规则 2（Event append-only），撤销通过追加本事件表达，**不修改也不删除**原先那条 `goal.completed`。本事件只适用于尚未归档的 `'completed'` 状态；已归档的 Goal 需先经恢复流程（→ `goal.restored`），不通过本事件直接处理（见 §3.9 一致性约束 5）。

**典型触发**：用户误勾了完成，随即撤销；用户认为目标其实还没真正达成，重新打开继续推进。

**不应触发**：已归档目标的恢复（→ `goal.restored`）；任务的取消完成（→ §7.1 `task.uncompleted`）；目标从未被标记完成（无 `completedAt` 可撤销，不触发）。

---

#### goal.archived（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ outcome, archivedAt, linkedTaskCount }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `outcome` | `string` | 否 | 归档类型；取值约束：`'completed'`（达成归档）/ `'abandoned'`（放弃归档）之一 |
| `archivedAt` | `string` | 否 | 归档时刻；取值约束：ISO 8601 带时区格式 |
| `linkedTaskCount` | `number` | 否 | 归档那一刻该 Goal 名下任务总数（快照值，不含已软删除的任务）；取值约束：整数，≥ 0 |

**说明**：Goal 生命周期正常终结时触发，`status` 变更为 `'archived'`。两条路径：**达成归档**（`outcome='completed'`）——目标已完成、不再需要出现在计划页主线清单里；**放弃归档**（`outcome='abandoned'`）——目标不再推进，但此前的投入与关联任务全部保留可查。归档**不改变**名下任何 Task 的 `goalId`、状态或归属，名下未完成的任务仍留在活动清单 / 今日待办中照常可用（见 §3.9 关键规则 8）。归档不是删除：归档视图中可展开该目标、查看其名下全部任务与累计投入（见 §8.13）。

**典型触发**：用户读完《人类简史》并把该目标标记完成后，将其归档收起；用户决定今年不再推进"考取驾照"，选择放弃归档。

**不应触发**：目标被标记完成但未归档（→ 只触发 `goal.completed`，两步分开）；目标被软删除（→ `goal.deleted`，删除与归档语义不同）；名下任务被归档（→ §7.1 `task.archived`）。

---

#### goal.restored（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ fromStatus, previousOutcome }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `fromStatus` | `string` | 否 | 恢复前的状态；取值约束：`'archived'` / `'deleted'` 之一 |
| `previousOutcome` | `string \| null` | 是 | 恢复前的 `outcome`（仅 `fromStatus='archived'` 时可能非 null）；取值约束：`null` / `'completed'` / `'abandoned'` 之一 |

**说明**：已归档或已软删除的 Goal 被恢复为 `'active'` 时触发，`archivedAt` / `outcome` / `deletedAt` / `deletedReason` 一并清空。这是误操作的回退路径——尤其是软删除的恢复：因为删除 Goal **不清空**名下任务的 `goalId`（见 §3.9 关键规则 6），恢复之后原有的全部归属关系与历史统计自动完整回来，不需要重新逐个关联任务。

**典型触发**：用户误删了一个主线目标，随即恢复；用户重新捡起一个此前放弃归档的目标继续推进。

**不应触发**：未归档、未删除的目标被取消完成（→ `goal.uncompleted`）；任务的恢复（→ §7.1 `task.restored`）。

---

#### goal.reordered（中长期主线目标功能批次）
...
| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `fromIndex` | `number` | 否 | 拖拽前在主线目标清单中的位置下标（0 起始）；取值约束：整数，≥ 0 |
| `toIndex` | `number` | 否 | 拖拽后的位置下标（0 起始）；取值约束：整数，≥ 0，且与 `fromIndex` 不同 |

**说明**：用户在计划页主线目标清单内拖拽调整目标顺序时触发，同步更新 `Goal.sortIndex`（步长策略见 §3.9 字段定义）。

**典型触发**：用户把最近想集中推进的目标拖到清单顶部。

**不应触发**：任务在活动清单内重排（→ §7.4 `task.reordered`）；今日待办内重排（→ §7.3 `dayPlan.taskReordered`）；合并组内成员重排（→ §7.19 `mergeGroup.reordered`）；`toIndex` 与 `fromIndex` 相同（未实际移动不触发）。

---

#### goal.deleted（中长期主线目标功能批次）

**顶层关联字段**：`goalId`。

**payload**：`{ deletedReason, affectedTaskCount }`

| 字段 | 类型 | 可空 | 含义与取值约束 |
|---|---|---|---|
| `deletedReason` | `string \| null` | 是 | 删除原因；取值约束：`null` / `'userDeleted'` / `'dataCleanup'` 之一，不允许自由文本 |
| `affectedTaskCount` | `number` | 否 | 删除那一刻 `goalId` 仍指向该 Goal 的任务数（快照值）；取值约束：整数，≥ 0 |

**说明**：Goal 被**软删除**时触发（写 `deletedAt`、`status` 改为 `'deleted'`）。按 §3.9 关键规则 6，删除**不清空**名下任务的 `goalId`——这些任务的 `goalId` 继续指向这条 tombstone，因此本事件**不触发**任何 `task.goalUnlinked`。`affectedTaskCount` 记录本次删除波及多少条历史归属，供事后判断影响面。

**产品层强制要求**：因为该操作影响面覆盖名下全部历史任务、并会使这些任务在 Goal 维度统计中转入"已删除目标"分组，实现端**必须以弹窗二次确认后才允许执行**（见 §3.9 关键规则 6）。

**典型触发**：用户删除一个建错的主线目标。

**不应触发**：目标归档（→ `goal.archived`，归档不是删除）；名下任务被删除（→ §7.1 `task.deleted`）；解除某个任务与目标的关联（→ §7.4 `task.goalUnlinked`）；物理删除（**任何情况下都不允许**，见 §3.9 关键规则 10）。
...
**合并番茄钟的计数口径（v4.3 改写）**：上述公式按 **Session 记录数**计数，不因 `Session.taskIds` 长度而重复计数——一次合并了 3 个任务的 focus Session 完成，本节口径下的全局有效番茄数 `+1`。**这个番茄归属于该合并组（MergeGroup），不归属于组内任何成员任务**：§8.5 的 Task 维度统计中，参与合并的成员任务有效番茄数**均不增加**（见 §3.3 关键规则 12、§8.5.1）。因此 v4.3 起，**任务维度有效番茄数之和不会大于本节的全局有效番茄总数**，两个口径可以相互对账。

> **与 v4.1 / v4.2 的差异**：旧版本规定"每个参与合并的任务各自 `+1` 个有效番茄"，并接受"任务维度加总 > 全局总数"。v4.3 已推翻该口径，理由见 §3.3 关键规则 12 的说明。实现端如果发现任何"各任务有效番茄之和大于全局总数"的现象，v4.3 起一律视为**缺陷**，不再是设计如此。
...
**定义**：某 Task 的有效番茄数 = 该 Task 下满足有效番茄条件（§8.3.1）、且 `mergeGroupId` 为 null 的标准 focus Session 数。**合并番茄钟场景（v4.3 改写）**下，由合并组触发的 focus Session（`mergeGroupId` 非 null）**不计入任何成员任务的有效番茄数**——那个番茄记在合并组身上（见 §3.3 关键规则 12、§8.3.1）。成员任务在该次专注中的收获，只体现为它在 `Session.taskSegments` 里那一段实际耗时，计入 §8.5.2 的 Task 专注总时长，不产生番茄计数。
...
&& Session.type === 'focus'
&& Session.status === 'completed'
&& Session.mergeGroupId === null     // v4.3：合并 Session 不计入任何成员任务
&& Session.deletedAt === null
```

**排除**（均不计入 Task 有效番茄数）：`focus.discarded`、`extraFocus`、`shortBreak`、`longBreak`、`extraRest`、**以及全部 `mergeGroupId` 非 null 的合并 focus**。
...
今日新增有效番茄数（某 Task）=
  count(Session where taskIds.includes(task.id) and type='focus' and status='completed'
        and mergeGroupId === null and deletedAt===null and appDate = 目标日期)

completedValidFocusCountForTask（历史累计有效番茄数）=
  count(Session where taskIds.includes(task.id) and type='focus' and status='completed'
        and mergeGroupId === null and deletedAt===null)   // 全时间段，不限日期
```

**任务维度加总 ≤ 全局总数（v4.3 改写）**：合并 Session 不计入任何成员任务的有效番茄数，因此把多个任务的有效番茄数相加，结果**不会超过** §8.3.1 的全局有效番茄总数；两者的差额恰好等于合并组贡献的番茄数（每个合并 Session 记 1 个）。对账关系：`Σ 各 Task 有效番茄数 + Σ 各 MergeGroup 有效番茄数 = 全局有效番茄总数`。

> **v4.1 / v4.2 的"任务维度加总 > 全局总数"口径已作废**，不得再作为预期现象向用户解释或在实现中保留。
...
```
// 取本 Task 在某条 Session 上的投入秒数：
// 合并 Session 取该 Task 在 taskSegments 里那条分段的 actualDuration；
// 非合并 Session 取整段 Session.actualDuration。
contribution(session, task) =
  session.mergeGroupId === null
    ? session.actualDuration
    : (session.taskSegments.find(s => s.taskId === task.id)?.actualDuration ?? 0)

Task 标准专注时长 =
  sum(contribution(session, task) where taskIds.includes(task.id)
      and type='focus' and status='completed' and deletedAt===null)

Task 额外专注时长 =
  sum(Session.actualDuration where taskIds.includes(task.id)
      and type='extraFocus' and deletedAt===null)   // extraFocus 不支持合并，无分段
...
> 跨多轮合并时，某成员的实际投入 = 所有相关 Session 中属于它的 `taskSegments.actualDuration` 之和；break 时间天然不在任何分段里，因此不会被计入。举例：第 1 个番茄的最后 3 分钟开始处理任务 D，休息 5 分钟，第 2 个番茄继续处理 D 共 7 分钟并完成 —— D 的实际投入时间 = 3 + 7 = 10 分钟，D 的有效番茄数 = 0，该 MergeGroup 的有效番茄数 = 2，中间那 5 分钟休息不计入 D。

> 历史迁移的合并 Session 若因没有可还原的成员分段事实而保留 `taskSegments: []`，其对每个成员的贡献按 `0` 计算；不得以 Session 总时长平均分摊，也不得事后猜测补齐。该历史缺口可能使成员维度合计小于全局或 MergeGroup 维度，但不会制造虚假投入事实。
...
- **合并番茄钟场景下（v4.3 改写）**，某成员任务在一次合并 Session 中的专注时长，取该 Session `taskSegments` 中属于它的那条分段的 `actualDuration`，**不是**整段 Session 的 `actualDuration`。因为分段之和精确等于 Session 的 `actualDuration`（见 §3.3 字段一致性约束 16），所以任务维度的时长加总**不会超过** §8.3.2 的全局标准专注时长，两个口径可以相互对账。
  > **v4.1 / v4.2 的旧口径已作废**：旧版本规定"整段 `actualDuration` 被每个成员各自完整计入一遍"，并接受跨维度求和大于真实投入。v4.3 起该现象一律视为缺陷。
...
预估准确 / 偏差统计样本 =
  ...
  && 该事件关联的 Session.mergeGroupId === null    // v4.3：排除合并组成员
```

- **合并组成员任务一律排除（v4.3 新增）**：成员在合并番茄里完成时 `completionSource='pomodoro'` 且 `validFocusCountAtCompletion=0`（见 §7.1），若把它当作样本，会把"预估 1、有效番茄 0"错误判成**初始预估偏大**，污染整个指标。判据取 `task.completed` 事件关联 `sessionId` 对应 Session 的 `mergeGroupId` 是否非 null，**不得**改用"`completionSource='pomodoro'` 即有有效番茄"这类推断。成员的真实投入用 §8.5.2 的分段时长呈现；时间口径的预估准确率如有需要另行设计，本版不提供。
...
**合并番茄钟场景（v4.3 改写）**：一次 Session 的 `taskIds` 可能包含分类不同的多个任务（如一次合并专注同时挂了一个"工作"任务和一个"副业"任务）。此时**按成员分段计入各自的分类桶**：每个成员把 `taskSegments` 里属于自己的那段 `actualDuration` 计入它自己 `category` 对应的桶，未分类成员计入"未分类"桶。因为分段之和精确等于 Session 的 `actualDuration`，**跨分类求和不会大于**该时间范围内的全局总专注时长（§8.3 口径），两者可以对账。

> **v4.1 / v4.2 的旧口径已作废**：旧版本规定"整段 `actualDuration` 被各自命中的分类桶完整计入一遍，跨分类求和可能大于全局总专注时长"。v4.3 起不再接受该现象。

---

#### 8.5.7 MergeGroup 维度统计（v4.3 新增）

v4.3 起合并组本身是番茄与专注时长的归属单位（§3.3 关键规则 12、§3.8 关键规则 10），因此它需要一套与 Task 平行的统计口径。统计页默认以合并组为一个条目展示，条目名取 `MergeGroup.title`（§3.8）——**必须用 `title` 区分不同合并组**，不得只显示"杂事番茄"而无法分辨是哪一组；成员各自的耗时与时间段需展开该条目才可见。

**MergeGroup 有效番茄数**：

```
某 MergeGroup 有效番茄数 =
  count(Session where mergeGroupId === group.id and type='focus'
        and status='completed' and deletedAt===null)
```

一次正常完成的合并 focus 给本组记 1 个有效番茄，与本轮 `taskIds` 里有几个成员无关；跨多轮时逐轮累加（两轮正常完成即为 2）。作废（`discarded`）的合并 focus 不计入。

**MergeGroup 专注时长**：合并 Session 的**完整** `actualDuration` 归属本组（不做分段拆分——分段只用于把时间分给成员，见 §8.5.2）：

```
MergeGroup 标准专注时长 =
  sum(Session.actualDuration where mergeGroupId === group.id
      and type='focus' and status='completed' and deletedAt===null)
...
**不会重复计入全局**：合并 Session 的整段时长在本节记给合并组、在 §8.5.2 按分段记给各成员，这是**同一段时间的两个不同展示维度**，不得把两者相加当作总投入。可加总的对账关系只有两条，实现必须能同时满足：

```
Σ 各 Task 有效番茄数 + Σ 各 MergeGroup 有效番茄数 = 全局有效番茄总数（§8.3.1）
Σ 各 Task 专注时长（合并部分按分段）        = 全局总专注时长（§8.3.4）
```

**MergeGroup 预估准确 / 预估偏差**：**完全复用** §8.5.3 独立 Task 的既有算法，不另写一套——只是把样本从 `task.completed` 换成 `mergeGroup.completed` 事件（§7.19）、把 `Task.estimateRounds` 换成 `MergeGroup.estimateRounds`、把快照换成该事件 payload 的 `validFocusCountAtCompletion`：

```ts
MergeGroup 预估准确 / 偏差样本 =
  mergeGroup.completed Event
  && validFocusCountAtCompletion != null
  && MergeGroup.estimateRounds.length >= 1
```

严格口径（只认初始预估、二次 / 三次预估一律判为不准确）与三分法（准确 / 偏大 / 偏小）的判定规则与 §8.5.3 逐字一致。仅 `status='completed'` 的合并组进入样本；`'dissolved'` 的组（中途拆散 / 取消合并，不代表做完）与仍在进行的组均不进入。
...
### 8.13 Goal（中长期主线目标）统计，v4.3 新增

本节定义中长期主线目标（Goal，§3.9）维度的统计口径，回答"我在这个长期目标上，累计投入了多少任务、多少番茄、多少时间，推进到哪一步了"。

**归属依据：任务的当前 `goalId`，不是历史快照。**

本节所有指标一律按 `Task.goalId` **当前的值**聚合，不使用任何历史归属快照。由此产生一个**必须明确告知实现端与用户的行为**：把一个早已完成的任务补绑到某个 Goal（或改绑到另一个 Goal），该 Goal 的累计投入会**立即把这条历史投入计入**，此前看到的数字随之变化。**这是预期行为，不是数据异常**——归属变更本身由 §7.4 `task.goalLinked` / `task.goalUnlinked` 留痕，任何数字变化都可追溯到具体的那次操作。

**Goal 不产生自己的 Session。** Goal 永远不参与计时（§3.9 关键规则 1），因此本节所有数值全部来自其名下 Task 的既有口径，本节不引入任何新的原始数据来源。

---

#### 8.13.1 Goal 关联任务数与完成任务数

**定义**：
- **关联任务数** = `goalId` 指向该 Goal、且未被软删除（`deletedAt` 为 null）的 Task 条数，含各种状态（active / completed / splitNeeded / archived）。
- **已完成任务数** = 上述集合中 `status='completed'`，或 `status='archived'` 且 `outcome='completed'` 的 Task 条数。

**已软删除的任务不计入**任一指标（遵循 §2.4 默认读取过滤）。

**进度**（供计划页进度条使用）= 已完成任务数 ÷ 关联任务数；关联任务数为 0 时进度视为 0，不做除零运算。

> **进度会回退，这是正确行为**：中长期目标的任务是边推进边补充的。已完成 3 个、共 4 个（75%）时新增 2 个任务，进度立即变成 3/6（50%）。展示时**必须同时给出"已完成 N / 共 M"的原始数字**，不得只给百分比——只给百分比会让用户以为进度倒退是数据出错。

---

#### 8.13.2 Goal 累计投入番茄数

**定义**：该 Goal 名下所有 Task 的有效番茄数之和（Task 有效番茄数口径见 §8.5.1）。

**合并番茄钟的处理**：按 §8.5.1 的 v4.3 口径，由合并组触发的 focus Session **不计入任何成员任务的有效番茄数**，因此**也不计入 Goal 的累计投入番茄数**。这意味着：一个只在"杂事番茄"里被推进过的任务，对其所属 Goal 的番茄数贡献为 0，但**时长贡献不为 0**（见 §8.13.3）。

> 这是刻意的取舍：番茄是"完整专注单元"的计数，一次合并番茄只是一个完整单元，无法拆给多个目标；而时间可以精确切分给各成员。**因此 Goal 维度应以投入时长为主指标、番茄数为辅助指标**，展示时不应把番茄数作为衡量长期目标推进程度的唯一依据。

---

#### 8.13.3 Goal 累计投入时长（主指标）

**定义**：该 Goal 名下所有 Task 的专注总时长之和（Task 专注总时长口径见 §8.5.2），即：

- 单任务专注（`mergeGroupId` 为 null）：取该 Session 的 `actualDuration`；
- 合并番茄钟：取该 Session `taskSegments` 中属于该任务的分段 `actualDuration`（**不是**整段 Session 时长，见 §8.5.2）；
- 同 §8.5.2，明细层必须可拆分为标准专注时长 / 额外专注时长 / 作废专注时长三部分，不得无差别合并。

**可加性保证**：因为合并 Session 的各分段之和精确等于该 Session 的 `actualDuration`（§3.3 一致性约束 16），且每个 Task 最多只能关联一个 Goal（§3.9 关键规则 4），所以：

```
Σ 各 Goal 投入时长 + 未关联任何 Goal 的任务投入时长 = 全局总专注时长（§8.3.4）
```

**跨 Goal 求和不会超过用户真实的专注时间**，多个 Goal 的投入时长可以直接相加对外呈现。这是 v4.3 改写合并番茄口径的核心目的之一——旧口径下同一段时间会被多个 Goal 各自完整计入，加总会显著超过真实投入，使该数字失去意义。

---

#### 8.13.4 Goal 最近推进时间

**定义**：该 Goal 名下所有 Task 关联的 focus Session 中，最晚一条的 `startedAt`；名下从未发生过任何 focus 时为 null。

**用途**：中长期目标最容易出的问题不是"没进展"而是"忘了它还在"。累计数字看不出"这条目标我已经三周没碰了"，因此计划页主线清单**应当展示本指标**（如"最近推进：18 天前"），使 Goal 起到提醒作用。

**取值口径**：包含 `status='discarded'` 的 focus（用户确实碰过它），包含 `extraFocus`，包含合并番茄钟中该任务作为成员参与的 Session（哪怕它的分段耗时为 0——用户当时确实把它排进了那个番茄）。本指标只回答"最后一次碰它是什么时候"，不衡量成果。

---

#### 8.13.5 已删除 Goal 的归属处理

按 §3.9 关键规则 6，软删除 Goal **不清空**名下任务的 `goalId`。因此统计层会遇到 `goalId` 指向一条已软删除 Goal 的任务，处理规则：

1. 这些任务**不并入"未关联任何 Goal"**分组——它们有明确归属，只是归属对象已被删除；
2. 统计层把它们归入一个独立的 **"已删除目标"** 分组，该分组不在计划页主线清单中展示；
3. 该分组的投入时长**仍计入** §8.13.3 的可加性等式左侧（否则等式不成立）；
4. Goal 被恢复（§7.20 `goal.restored`）后，这些任务自动回到该 Goal 名下，历史统计完整恢复，无需重新关联。

---

#### 8.13.6 Goal 分类与任务分类的关系

`Goal.category` 与其名下 `Task.category` 是两条**互相独立**的取值（§3.9 关键规则 7、§3.1 关键规则 14）。因此：

- **按分类统计投入时长**（§8.5.6）一律以 `Task.category` 为准，**不得**用所属 Goal 的 `category` 替代或覆盖；
- **按 Goal 统计**（本节）一律以 `Task.goalId` 为准，与任何一方的 `category` 无关；
- 两套统计的结果**不要求一致**：一个 `category='work'` 的 Goal，其投入时长完全可能有一部分落在"生活"分类桶里。这不是数据错误，不得为了让两个数字对齐而做任何强制校验或追溯改写。
...
- 范围：MergeGroup 实体结构、Task/Session/Event 的相应字段扩展、8 个 `mergeGroup.*` 事件、统计口径补充（§8.3、§8.5），在 v4.1 定义、v4.3.2 收口。实质改动包括：番茄归组、成员按 `taskSegments` 记时间；MergeGroup 成为与独立 Task 同级的番茄执行任务并具有 `completed` 成功终态；新增 `title` / `renamed`；完成时清空当前归属指针并保存终结快照；completed 组参与预估准确率。
- 本轮**只定义数据结构与规则**，真实 UI 交互（清单页"拖到正中间=合并"的手势、计时页合并卡片展示、"结束 / 追加预估"选择界面）未在本文档范围内实现，属于后续独立的施工计划。

---

### 10.8 中长期主线目标（新增功能批次，范围）

- v4.3 新增的"中长期主线目标"（Goal，见 §3.9、§7.20）与"合并番茄钟"同属 Phase 1–3 封板之后的**独立功能批次**，不套用 P1–P5 编号，事件表统一标注为"中长期主线目标功能批次"。
- 范围：Goal 实体结构、`Task.goalId` 字段、9 个 `goal.*` 事件、2 个关联类事件（§7.4 `task.goalLinked` / `task.goalUnlinked`）、Goal 维度统计口径（§8.13）。
- **同批次的破坏性改动**：子任务机制（`Task.parentId`、§7.2 `subtask.*` 4 个事件、§7.4 `task.reparented`）整体废除，见 §3.1「关于 `parentId` 的废弃说明」。废除时产品尚未正式投入使用，无历史数据需要迁移。
- 本轮**只定义数据结构与规则**，真实 UI 交互（计划页四模块布局、主线目标清单与进度条、批量关联任务到目标、统计页 Goal 维度视图）未在本文档范围内实现，属于后续独立的施工计划。
- **实现端连带工作提示**：本批次改动会波及已上线的多端同步实现——新增 Goal 实体需要建表、配置行级安全策略与上传 / 下载映射；废除 `parentId` 需要同步清理相应映射与云端历史数据。具体改造不在本文档范围内。
...
> **【V43-1】～【V43-6】均已确认并落实，§14 不再保留 v4.3 待确认项。**
>
> - **V43-1**：合并成员完成时 `completionSource='pomodoro'`，表示在番茄流程中完成，不表示成员获得有效番茄。
> - **V43-2**：合并成员 `validFocusCountAtCompletion=0`，不新增投入快照；真实投入只读 `taskSegments`。
> - **V43-3**：discarded 合并 focus 仍写 `taskSegments`，计入成员作废专注时长，不计入有效番茄。
> - **V43-4**：保留 Goal `outcome='abandoned'`，与达成归档、软删除分别处理。
> - **V43-5**：Goal 当前不增加期限 / 截止日期字段；未来若需要，另行扩展字段、事件与统计口径。
> - **V43-6**：`Task.mergeGroupId` 只表示当前所属；MergeGroup 进入 completed / dissolved 终态时清空全部成员指针。完成事务同时写 Group 终态、`completedAt`、成员指针清空与 `mergeGroup.completed` Event；历史归属由 Group、Session 分段与 Event 关联承载。
>
> 另保留 v4.3.1 已确认的分层规则：每轮开轮前至少 2 个未完成成员；已开跑 Session 的存储约束保持 `taskIds ≥1`，允许缩到 1 后正常收尾。`mergeGroup.renamed` 与 `mergeGroup.completed` 均保留，§7.19 共 8 个 `mergeGroup.*` 事件。
