...
> **正式基准文档。本文件取代 v1 / v2 / v2.1 / v3 / v4.0（`data-layer-spec-v4.md`）/ v4.1（`data-layer-spec-v4.1.md`）。后续所有开发仅参考本文件（v4.2.0）。**
> 版本：4.2.0 | 制定日期：2026-05-29 | 最近修订：2026-08-08（新增任务分类能力：Task `category`/`categorySource` 字段（§3.1）、DayPlan `mainCategory`/`mainCategorySource` 今日主线分类字段（§3.2）、Settings `taskCategoryWindows` 自动打标时间窗口配置（§3.7）、`task.categoryChanged` / `dayPlan.mainCategoryChanged` / `settings.taskCategoryWindowsChanged` 三个事件（§7.1、§7.3、§7.12）、按分类统计投入时长口径（§8.5.6），独立成 `data-layer-spec-v4.2.md` 文件，`data-layer-spec-v4.1.md` 保留原内容作历史存档）
...
| **v4.2.0（本文）** | 新增"任务分类"能力：Task `category` / `categorySource` 字段（§3.1）、DayPlan `mainCategory` / `mainCategorySource` 今日主线分类字段（§3.2）、Settings `taskCategoryWindows` 自动打标时间窗口配置（§3.7）、`task.categoryChanged` / `dayPlan.mainCategoryChanged` / `settings.taskCategoryWindowsChanged` 三个事件（§7.1、§7.3、§7.12）、按分类统计投入时长口径（§8.5.6） | **唯一有效版本** |

**声明**：自本文件制定之日起，v1 / v2 / v2.1 / v3 / v4.0 / v4.1 均降级为历史归档，仅供溯源查阅，**不再作为任何开发决策的依据**。如历史版本内容与本文有出入，以本文为准。v4.0.0 与 v4.1.0 的内容已完整内嵌入本文（v4.2.0），未被 v4.2 修订的部分保持不变。
