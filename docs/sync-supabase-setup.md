...
- [x] 建表 + 权限规则（`supabase/schema.sql`）——已执行，用匿名请求核对过 7 张表都建好了
- [ ] 登录跳转网址白名单——**还差这一步，需要你去 Supabase 后台配置**（见下方第五节）
...
侧边栏底部已经能看到"未登录，数据只存在本机"的提示和邮箱输入框。实测过一次：

- 用一个不在白名单里的邮箱提交，Supabase 正确拒绝（返回"Signups not allowed for this instance"），
  界面上也正确显示了这条拒绝信息——说明白名单和前端的对接是通的。
- 没有真的用你的白名单邮箱去发送登录链接（那样会真的发一封邮件到你邮箱），这一步留给你自己在
  实际部署的网站上完成，因为登录链接的邮件也只有你能点。

## 五、还差的最后一步：登录跳转网址白名单

网站已经部署在 `https://toy.viyi.cc`，但 Magic Link 登录邮件里的跳转链接需要在 Supabase 后台
把这个网址加入白名单，否则点邮件里的链接会跳转失败（这一步我没有权限操作，需要你去后台点一下）：

1. Supabase 控制台 → **Authentication** → **URL Configuration**。
2. **Site URL** 填 `https://toy.viyi.cc`。
3. **Redirect URLs** 里加一条 `https://toy.viyi.cc/**`（星号是为了兼容登录后可能带的参数）。
4. 保存。

配置好之后，建议你自己在 `https://toy.viyi.cc` 上用白名单邮箱走一遍真实登录：输入邮箱 → 点"发送" →
去邮箱点登录链接 → 确认能跳回网站且侧边栏显示"已同步"或你的邮箱。这一步我做不了（点邮件链接
只有你能做），麻烦你验证后告诉我结果。
